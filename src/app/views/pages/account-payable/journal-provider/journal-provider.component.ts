...
@Component({
  selector: 'kt-journal-provider',
  templateUrl: './journal-provider.component.html',
  styleUrls: ['./journal-provider.component.scss']
})
export class JournalProviderComponent implements OnInit {
  ...
  solist() {
    ...
    this.accountPayableService.getByRange(obj).subscribe(
      (response: any) => {   
  ...
  onChangecust() {
    ...
    this.providerService.getBy({ vd_addr:cm_addr }).subscribe(
      (res: any) => {
  ...
  handleSelectedRowsChangedcust(e, args) {
    const controls = this.soForm.controls;
      ...
      args.rows.map((idx) => {
       ...
       controls.cust.setValue(item.vd_addr);

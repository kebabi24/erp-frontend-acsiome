...
@Component({
  selector: 'kt-create-charge-payable-detail',
  templateUrl: './create-charge-payable-detail.component.html',
  styleUrls: ['./create-charge-payable-detail.component.scss']
})
export class CreateChargePayableDetailComponent implements OnInit {
  ...
  createForm() {
    ...
    //    so__chr01: [this.accountPayable.au__chr01],
        ...
        aud_fc_code:  [null, Validators.required],      
    ...
    this.bankService.getBy({ bk_code : controls.au_bank.value }).subscribe(
      (resp: any) => {
        ...
        if (resp.data.bank.bk_balance >= Number(controls.au_amt.value)) {
          ...
          let bkh = this.prepareBkh()
          this.mvdataset.push({
        id:1,
        aud_fc_code : controls.aud_fc_code.value,
        aud_desc : controls.desc.value,
        aud_amt : Number(controls.au_amt.value)


          })
          this.addAS(as, bkh,this.mvdataset);
   ...
   * @param _as: as
   */
  addAS(_au: any, _bkh:any,detail: any) {
    ...
    .addFCD({ accountUnplanifed: _au, BKH:_bkh,accountUnplanifedDetail: detail})
      .subscribe(
        (reponse: any) => (as = reponse.data,this.nbr = reponse.data),
        (error) => {
        ...
        () => {
          ...
          this.loadingSubject.next(false);
          this.reset()
          this.router.navigateByUrl("/account-payable/create-charge-payable-detail");
          this.reset()
        }
      );
  }
...
handleSelectedRowsChangedc(e, args) {
  ...
  if (Array.isArray(args.rows) && this.gridObjc) {
    this.selectedc = args.rows.map((idx: number) => {
      ...
      controls.aud_fc_code.setValue(item.fc_code || "");
      controls.desc.setValue(item.fc_desc || "");
...
next2000(){
  ...
  calcule(){
    ...
    const total = Number(controls.bkh_2000.value) * 2000 + Number(controls.bkh_1000.value) * 1000
      ...
      controls.au_amt.setValue(total)            
  }     
  ...
  printpdf() {
    ...
    doc.setFontSize(9);
    doc.text("Personne    : " + controls.au_vend.value + " "+ controls.name.value, 5, initialY + 10);
   ...
   doc.text("Dépense    : " + controls.aud_fc_code.value + " "+ controls.desc.value, 5, initialY + 20);
  
   if(controls.au_po.value != null) {doc.text("Observation    : " + controls.au_po.value , 5, initialY + 25)};
    doc.setFontSize(9);
...
var i = 45
    ...
    //   doc.line(2, i - 5, 2, i);
       ...
       doc.text("Total Dépense"  , 4, i+75 ) ; doc.text( String(Number(controls.au_amt.value).toFixed(2)),45,i+ 75); 

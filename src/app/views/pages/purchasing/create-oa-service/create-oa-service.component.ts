...
@Component({
  selector: 'kt-create-oa-service',
  templateUrl: './create-oa-service.component.html',
  styleUrls: ['./create-oa-service.component.scss']
})
export class CreateOaServiceComponent implements OnInit {
  ...
  servicename:any
    ...
    createForm() {
        ...
        this.reqForm = this.reqFB.group({
          ...
          rqm_job: [this.requisition.rqm_job , Validators.required],
        //   seqname: [null],
...
onchangeser(){
  ...
  const index = this.rqm_job.findIndex(detaill=>{
    return detaill.code_value == controls.rqm_job.value
 })
  this.servicename = this.rqm_job[index].code_cmmt
  this.sequencesService
  .getByService({usgseq_service: controls.rqm_job.value})
  .subscribe((response: any) => {
        ...
        this.sequencesService
            ...
            .subscribe((response: any) => {
                ...
                if (response.data.length == 0) {
                   ...
                   this.seqname = response.data[0].seq_desc
                }
...
this.addReq(req, this.dataset)
    ...
    prepareReq(): any {
        ...
        const _req = new Requisition()
          ...
          _req.rqm_job =  controls.rqm_job.value
...
console.log("controls.rqm_vend.value",controls.rqm_rqby_userid.value)
    ...
    prepareGrid3() {
        ...
        // fill the dataset with your data
        const controls = this.reqForm.controls
        this.employeService.getBy({emp_job:controls.rqm_job.value}).subscribe((response: any) => (this.users = response.data));
    }
    open3(content) {
        this.prepareGrid3()
        this.modalService.open(content, { size: "lg" })
    ...
    doc.text(this.docs[0].date02, 180, 27);
        ...
        doc.text("Date: " + date.toLocaleDateString() , 160, 45);
          ...
          doc.text("Service       : " + controlss.rqm_job.value + " " + this.servicename, 20, 60);
          doc.text("Type D'Achat : " +  " " + this.seqname, 20, 65);
         ( controlss.rqm_vend.value!=null &&  controlss.rqm_vend.value != "") ? doc.text("Fournisseur : " + controlss.rqm_vend.value + " " + controlss.name.value, 20, 70) : doc.text("Fournisseur : " , 20, 70) ;

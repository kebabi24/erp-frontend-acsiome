...
export class CustomerSettingsComponent implements OnInit {
 ...
 /* onchangelabel(){
  ...
  onChangeField(){
    ...
    if(controls.code_fldname.value == 'cm_cr_terms') {
      this.isTerms = true
  ...
  addCode(_code: Code) {
      ...
      this.codeService.add(_code).subscribe(
          ...
          () => {
              ...
              this.loadingSubject.next(false)
               this.router.navigateByUrl("/customers/customer-settings")
              this.reset()
          }
      )
  }
  ...
  goBack() {
      this.loadingSubject.next(false)
      const url = `/customers/customer-settings`
...
prepareGrid() {
  ...
  this.columnDefinitions = [
      ...
      filter: {
        collection: [ { value: 'cm_type', label: 'Type' }, { value: 'cm_region', label: 'Région' },{ value: 'cm_shipvia', label: 'Mode Expédition' }, { value: 'cm_promo', label: 'Groupe Promotion' },{ value: 'cm_lang', label: 'Langue' }, { value: 'check_form', label: 'Mode de Paiement' },{ value: 'cm_cr_terms', label: 'Délai de Paiement' },{ value: 'cm_class', label: 'Classe' },],

...
@Component({
  selector: 'kt-customer-settings-edit',
  templateUrl: './customer-settings-edit.component.html',
  styleUrls: ['./customer-settings-edit.component.scss']
})
export class CustomerSettingsEditComponent implements OnInit {
  code: Code
  codeForm: FormGroup
  hasFormErrors = false
...
check_form: any[] = []
  constructor(
      ...
      this.codeService
      .getBy({ code_fldname: "cm_pay_method" })
  ...
  ngOnInit(): void {
      ...
      this.activatedRoute.params.subscribe((params) => {
          ...
          this.codeService.getOne(id).subscribe((response: any)=>{
            ...
            if(this.codeEdit.code_fldname == 'cm_cr_terms') { this.isTerms = true }
  ...
  addCode(_code: Code) {
      ...
      this.codeService.update(this.codeEdit.id, _code).subscribe(
          ...
          () => {
              ...
              this.loadingSubject.next(false)
              this.router.navigateByUrl("/customers/customer-settings-list")
          }
      )
  }
  ...
  goBack() {
      this.loadingSubject.next(false)
      const url = `/customers/customer-settings-list`

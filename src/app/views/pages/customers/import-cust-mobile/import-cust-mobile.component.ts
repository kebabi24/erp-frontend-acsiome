...
@Component({
  selector: 'kt-import-cust-mobile',
  templateUrl: './import-cust-mobile.component.html',
  styleUrls: ['./import-cust-mobile.component.scss']
})
export class ImportCustMobileComponent implements OnInit {
...
fldname;
  ...
  ngOnInit() {
      ...
      this.deviseService.getBy({ cu_active:  true }).subscribe(
        (response: any) => {
      controls.cm_curr.setValue(response.data.cu_curr)
        })
...
handleSelectedRowsChangedcust(e, args) {
  const controls = this.addressForm.controls;
  const controlsc = this.customerForm.controls;
  if (Array.isArray(args.rows) && this.gridObjcust) {
    args.rows.map((idx) => {
      ...
      this.customer = item;
      controls.ad_addr.setValue(item.customer_code || "");
      controls.ad_name.setValue(item.customer_name || "");
      controls.ad_line1.setValue(item.address_one || "");
      controls.ad_phone.setValue(item.customer_phone_one || "");
      controlsc.cm_bill.setValue(item.customer_code || "");
     this.onChangeCode()
    });
  }
}

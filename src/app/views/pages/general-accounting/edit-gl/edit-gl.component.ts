...
export class EditGlComponent implements OnInit {
  ...
  entity: String;
  message = "";
  title: String = 'Modifier Ecriture Comptable - '
  glEdit;
  dateglt: any;
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((params) => {
        const id = params.id
        this.generalLedgerService.getBy({glt_ref:id}).subscribe((response: any)=>{
          this.glEdit = response.data[0]
          this.dataset = response.data
          this.dateglt = new Date(this.glEdit.glt_effdate)
            this.dateglt.setDate(this.dateglt.getDate() )
          this.initCode()
          this.loadingSubject.next(false)
          this.title = this.title + this.glEdit.gl_ref
        })
    })
    this.initGrid()
  }
  initCode() {
   ...
   createForm() {
    ...
    glt_ref: [{value: this.glEdit.glt_ref, disabled:true}] , 
    glt_effdate: [{
      year: this.dateglt.getFullYear(),
      month: this.dateglt.getMonth()+1,
      day: this.dateglt.getDate()
    }],  
    glt_curr: [this.glEdit.glt_curr, Validators.required],  
    glt_ex_rate: [this.glEdit.glt_ex_rate, Validators.required],  
    glt_ex_rate2: [this.glEdit.glt_ex_rate2, Validators.required],  
    glt_batch: [this.glEdit.glt_batch, Validators.required], 
    glt_dy_code: [this.glEdit.glt_dy_code],  
...
onSubmit() {
  ...
  if (mnt  != Number(controls.glt_batch.value) ) {
   ...
   for (var i = 0; i < this.dataset.length; i++) {
     ...
     ctr = ctr + Number(this.dataset[i].glt_amt);
...
prepareGl(): GeneralLedger {
  ...
  _gl.glt_ref      = controls.glt_ref.value;
...
for (let data of details) {
  ...
  this.loadingSubject.next(true)
  this.generalLedgerService.update(this.glEdit.glt_ref, { generalLedger: _gl, Detail: details} ).subscribe(
      ...
      () => {
          ...
          this.loadingSubject.next(false)
          this.reset();
          this.router.navigateByUrl("/general-accounting/list-gl")
      }
  )
}

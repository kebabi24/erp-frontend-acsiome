...
@Component({
  selector: 'kt-edit-population',
  templateUrl: './edit-population.component.html',
  styleUrls: ['./edit-population.component.scss']
})
export class EditPopulationComponent implements OnInit {
  ...
  popEdit: any
  title: String = 'Modifier Population - '
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((params) => {
        const id = params.id
        this.populationemployeService.getOne(id).subscribe((response: any)=>{
          this.popEdit = response.data[0]
          this.selectedCustomers = response.data
          this.initCode()

          this.loadingSubject.next(false)
          this.title = this.title + this.popEdit.pop_code
  ...
  createPopulationForm() {
    ...
    this.populationForm = this.formBuilder.group({
      pop_code: [ this.popEdit.pop_code, Validators.required],
      pop_desc: [ {value : this.popEdit.pop_desc,disabled : !this.isExist}, Validators.required],
  ...
  let items: any[] = [];
    ...
    this.employes.forEach((element) => {
      this.selectedCustomers.forEach((element2) => {
        if (element.emp_addr === element2.pop_emp) {

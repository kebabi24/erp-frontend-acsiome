...
import { AccountShiper,
 AccountShiperService,Employe, EmployeService, JobService , CodeService, SiteService, UsersService,ItemService} from "../../../../core/erp"


@Component({
  selector: 'kt-edit-student',
...
export class EditStudentComponent implements OnInit {
  ...
  isExist = false
  employeEdit: any
  title: String = 'Modifier Etudiant - '
...
createForm() {
  ...
  this.empForm = this.empFB.group({
      emp_addr: [this.employeEdit.emp_addr],
      emp_fname: [this.employeEdit.emp_fname,
        Validators.required
...
initmvGrid() {
  ...
  .subscribe((response: any) => {this.mv = response.data
    for (let m of this.mv) {
      ...
      console.log(result)
      if (result!=null){m.emps_amt = result.emps_amt}
    }
...
prepareGrid() {
    ...
    this.columnDefinitions = [
        ...
        }, 
        {
          ...
          onCellChange: (e: Event, args: OnEventArgs) => {
          this.paid = false
           if(args.dataContext.paid ==0){args.dataContext.rest = 0
            alert('facture totalement reglée')
           }
          },
        },
        
        {
          ...
          formatter: (row, cell, value, columnDef, dataContext) => {
            ...
            return `
                 <a class="btn btn-sm btn-clean btn-icon mr-2" title="Réglement">
          ...
          onCellClick: (e: Event, args: OnEventArgs) => {
            ...
            _as.as_nbr = args.dataContext.as_nbr;
            _as.as_ship = args.dataContext.as_ship;
            _as.as_cust = args.dataContext.as_cust;
            _as.as_curr = args.dataContext.as_curr;
            _as.as_app_owner = controls.emp_fname.value + ' ' + controls.emp_lname.value;
            _as.as_effdate = date
                              ? `${date.year}/${date.month}/${date.day}`
                              : null;
            _as.as_type = 'P';
            _as.as_bank = 'CAISSE';
            _as.as_pay_method = 'ES';
            _as.as_check = '';
            _as.as_amt = (-1) * args.dataContext.rest;
            _as.as_applied = (-1) * args.dataContext.rest;
            _as.as_po = null;
    ...
    this.dataset = []
    const controls = this.empForm.controls
    console.log(controls.emp_fname.value + ' ' + controls.emp_lname.value)
      this.accountshipperService
    .getBy({as_app_owner: controls.emp_fname.value + ' ' + controls.emp_lname.value})
    .subscribe(

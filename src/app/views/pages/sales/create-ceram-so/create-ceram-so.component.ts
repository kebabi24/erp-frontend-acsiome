...
controls.qtyCart.setValue(qty)
  ...
  createPayForm() {
    ...
    //    so__chr01: [this.accountOrder.ao__chr01],
        ...
        ao_pay_method: ["ES", Validators.required],
...
console.log(tht , tva , timbre,ttc)
  ...
  doc.setFontSize(12);
  doc.text("Commande N° : " + nbr, 80, 40);
  doc.setFontSize(12);
  //console.log(this.customer.address.ad_misc2_id);
  doc.text("Code        : " + this.customer.cm_addr, 10, 45);
  doc.text("Date        : " + this.soo.so_ord_date, 90, 45);
  doc.text("Nom         : " + this.customer.address.ad_name, 10, 50);
  // doc.text("Adresse       : " + this.customer.address.ad_line1, 20, 75);
  if(controlss.chr01.value!= null) {
  doc.text("Client      : " + controlss.chr01.value, 10, 55); }
  if(controlss.chr02.value != null) {
  doc.text("Tel         : " + controlss.chr02.value, 10, 60);}
  if(this.soo.so_rmks != null){
  doc.text("Observation : " + this.soo.so_rmks, 10, 65);}
  ...
  doc.text("LN", 12.5, 73.5);
  doc.line(20, 70, 20, 75);
  // doc.text("Code Article", 22, 73.5);
  // doc.line(45, 70, 45, 75);
  doc.text("Désignation", 22, 73.5);
  doc.line(60, 70, 60, 75);
  doc.text("M", 63, 73.5);
  doc.line(75, 70, 75, 75);
  doc.text("C", 77, 73.5);
  doc.line(90, 70, 90, 75);
  doc.text("P", 92, 73.5);
  doc.line(105, 70, 105, 75);
  doc.text("PU", 107, 73.5);
  doc.line(120, 70, 120, 75);
  // doc.text("REM", 172, 103.5);
  // doc.line(181, 100, 181, 105);
  doc.text("THT", 122, 73.5);
  ...
  doc.line(145, 70, 145, 75);
      ...
      if (this.dataset[j].sod_desc.length > 20) {
        let desc1 = this.dataset[j].sod_desc.substring(20);
        let ind = desc1.indexOf(" ");
        desc1 = this.dataset[j].sod_desc.substring(0, 20 + ind);
        let desc2 = this.dataset[j].sod_desc.substring(20 + ind);
  doc.setFontSize(9);
        ...
        doc.text(desc1, 22, i - 1);
        doc.line(60, i - 5, 60, i);
        doc.text(String(this.dataset[j].sod_qty_ord.toFixed(2)), 73, i - 1, { align: "right" });
        doc.line(75, i - 5, 75, i);
        doc.text(String(this.dataset[j].sod_qty_chg.toFixed(2)), 88, i - 1, { align: "right" });
        doc.line(90, i - 5, 90, i);
        doc.text(String(this.dataset[j].sod_qty_qote.toFixed(2)), 103, i - 1, { align: "right" });
        doc.line(105, i - 5, 105, i);
        doc.text(String(Number(this.dataset[j].sod_price).toFixed(2)), 118, i - 1, { align: "right" });
        
        // doc.line(170, i - 5, 170, i);
        // doc.text(String(this.dataset[j].sod_disc_pct) + "%", 173, i - 1);
        doc.line(120, i - 5, 120, i);
        doc.text(String((this.dataset[j].sod_price * ((100 - this.dataset[j].sod_disc_pct) / 100) * this.dataset[j].sod_qty_ord).toFixed(2)), 143, i - 1, { align: "right" });
        doc.line(145, i - 5, 145, i);
        // doc.line(10, i, 200, i );
  
        i = i + 5;
  
        doc.text(desc2, 22, i - 1);
        ...
        // doc.text(this.dataset[j].sod_part, 22, i - 1);
        // doc.line(45, i - 5, 45, i);
        doc.text(this.dataset[j].sod_desc, 22, i - 1);
        doc.line(60, i - 5, 60, i);
        doc.text(String(this.dataset[j].sod_qty_ord.toFixed(2)), 73, i - 1, { align: "right" });
        doc.line(75, i - 5, 75, i);
        doc.text(String(this.dataset[j].sod_qty_chg.toFixed(2)), 88, i - 1, { align: "right" });
    ...
    doc.line(70, i + 7, 145, i + 7);
    doc.line(70, i + 14, 145, i + 14);
    // doc.line(130, i + 21, 200, i + 21);
    // doc.line(130, i + 28, 200, i + 28);
    // doc.line(130, i + 35, 200, i + 35);
    doc.line(70, i + 7, 70, i + 14);
    doc.line(100, i + 7, 100, i + 14);
    doc.line(145, i + 7, 145, i + 14);
    doc.setFontSize(12);

    doc.text("Total ", 110, i + 12, { align: "left" });
    doc.text("Versé ", 110, i + 19, { align: "left" });
    // doc.text("TVA", 140, i + 19, { align: "left" });
    // doc.text("Timbre", 140, i + 26, { align: "left" });
    // doc.text("Total TC", 140, i + 33, { align: "left" });

    doc.text(String(Number(controls.tht.value).toFixed(2)), 143, i + 12, { align: "right" });
    doc.text(String(Number(controlsao.ao_amt.value).toFixed(2)), 143, i + 19, { align: "right" });
    ...
    if (mt.length > 95) {
      ...
      doc.text("Arretée la présente Commande a la somme de : " + mt1, 20, i + 30);
      doc.text(mt2, 20, i + 35);
    } else {
      doc.text("Arretée la présente Commande a la somme de : " + mt, 20, i + 30);
    }
    doc.addPage()
    var img = new Image();
    // img.src = "./assets/media/logos/companylogo.png";
  ...
  doc.setFont("Times-Roman");
  doc.setFontSize(12);
  doc.text("Commande Bureau N° : " + nbr, 80, 40);
  doc.setFontSize(12);
  //console.log(this.customer.address.ad_misc2_id);
  doc.text("Code        : " + this.customer.cm_addr, 10, 45);
  doc.text("Date        : " + this.soo.so_ord_date, 90, 45);
  ...
  doc.setFontSize(9);
    ...
    doc.setFontSize(12);
    let mtB = NumberToLetters(Number(controls.tht.value).toFixed(2), "Dinars Algérien");

    if (mtB.length > 95) {
      let mt1 = mtB.substring(90);
      let ind = mt1.indexOf(" ");

      mt1 = mtB.substring(0, 90 + ind);
      let mt2 = mtB.substring(90 + ind);

...
@Component({
  selector: 'kt-create-direct-so-seq',
  templateUrl: './create-direct-so-seq.component.html',
  styleUrls: ['./create-direct-so-seq.component.scss']
})
export class CreateDirectSoSeqComponent implements OnInit {
...
error = false;
              ...
              this.locationDetailService.getByOne({ ld_site: resp.data.pt_site, ld_loc: resp.data.pt_loc, ld_part: args.dataContext.sod_part, ld_lot: null }).subscribe(
                (response: any) => {
                  ...
                  this.inventoryStatusService.getAllDetails({isd_status: this.lddet.ld_status, isd_tr_type: "ISS-SO" }).subscribe((resstat:any)=>{
                    ...
                    this.gridService.updateItemById(args.dataContext.id,{...args.dataContext , desc: resp.data.pt_desc1 , desc2: resp.data.pt_desc2 ,sod_site:resp.data.pt_site, sod_loc: resp.data.pt_loc, sod_serial: null,
      ...
      if (id) {
        ...
        this.quoteService.findByOne({ id }).subscribe(
          (res: any) => {
            ...
            controls.so_cust.setValue(this.qoServer.so_cust);
            controls.so_bill.setValue(this.qoServer.so_bill);
            controls.so_po.setValue(this.qoServer.so_nbr);
            controls.so_curr.setValue(this.qoServer.so_curr);
            this.customersService
                  .getBy({ cm_addr: this.qoServer.so_cust })
              ...
              this.gridService.addItem(
                {
                  ...
                  sod_part: detail.sod_part,
                  cmvid: "",
                  desc: detail.item.pt_desc1,
                  desc2:detail.item.pt_desc2,
                  sod_qty_ord: detail.sod_qty_ord,
                  sod_um: detail.sod_um,
                  sod_price: detail.sod_price,
                  sod_disc_pct: detail.sod_disc_pct,
  ...
  //    so__chr01: [this.saleOrder.so__chr01],
      ...
      // so_bank:[this.saleOrder.so_bank],
      // so_prepaid:[this.saleOrder.so_prepaid],
  ...
  //    so__chr01: [this.saleOrder.so__chr01],
      tbt: [{value: 0.00 , disabled: true}],
      remise: [{value: 0.00 , disabled: true}],
      tht: [{value: 0.00 , disabled: true}],
      tva: [{value: 0.00 , disabled: true}],
      tva09: [{value: 0.00 , disabled: true}],
      tva19: [{value: 0.00 , disabled: true}],
  ...
  onChangeSeq() {
      ...
      this.sequencesService
          .getBy({seq_seq: controls.so_category.value, seq_type: 'SO'})
  ...
  prepareSo(): any {
    ...
    // _so.so_bank = controls.so_bank.value;
    // _so.so_prepaid = controls.so_prepaid.value;
  ...
  addSo(_so: any, detail: any) {
    ...
    this.saleOrderService
      ...
      .subscribe(
        (reponse: any) => (this.soo = reponse.data),
        (error) => {
        ...
        () => {
          ...
          if(controls.print.value == true) { 
            this.printpdf(this.soo.so_nbr)
            //this.mt = controls.so_prepaid.value;
            
            // if(controls.aide.value == false){
            //   if(controls.so_cr_terms.value == 'CI'){this.nom = controls.so_bank.value}
            //   else{this.nom = controls.name.value}
              
            // }
            // else{this.nom = 'CNL'}
            // this.printpv(so.so_nbr)} // printSO(this.customer, this.dataset, so);
          this.reset()
          this.loadingSubject.next(true);
          this.loadingSubject.next(false);
          this.router.navigateByUrl('/', { skipLocationChange: true }).then(() => {
            this.router.navigateByUrl("/sales/create-direct-so-seq");
            this.reset()
          });
  ...
  onChangeOC() {
    const controls = this.soForm.controls;
    const so_nbr = controls.so_po.value;
   
    this.quoteService.findByOne({ so_nbr: so_nbr }).subscribe(
      (res: any) => {
        const { quoteOrder, details } = res.data;
        console.log(quoteOrder)
        controls.so_cust.setValue(quoteOrder.so_cust)
        controls.so_bill.setValue(quoteOrder.so_bill);
        controls.so_curr.setValue(quoteOrder.so_curr)
        controls.so_cr_terms.setValue(quoteOrder.so_cr_terms)
        controls.so_taxable.setValue(quoteOrder.so_taxable)
        this.customersService.getBy({ cm_addr: quoteOrder.so_cust }).subscribe(
          (res: any) => {
            //console.log(res);
            const { data } = res;
      ...
      (error) => {
        this.message = `Demande avec ce numero ${so_nbr} n'existe pas`;
        this.hasFormErrors = true;
      },
      () => {}        
}
...
changeTax(){
      ...
      args.rows.map((idx) => {
        ...
        this.locationDetailService.getByOne({ ld_site: item.pt_site, ld_loc: item.pt_loc, ld_part: item.pt_part, ld_lot: null }).subscribe(
          (response: any) => {
           ...
           if (this.lddet != null) { 
            this.inventoryStatusService.getAllDetails({isd_status: this.lddet.ld_status, isd_tr_type: "ISS-SO" }).subscribe((resstat:any)=>{
              ...
              updateItem.sod_qty_ord = 0
              this.site = item.pt_site
              this.calculatetot()
         ...
         this.calculatetot()
          ...
          this.loctype = item.pt_part_type
          this.size = item.pt_size
      }) 
  ...
  handleSelectedRowsChanged5(e, args) {
    ...
    const so_nbr = controls.so_po.value;
    const so_cust = controls.so_cust.value;

    if (Array.isArray(args.rows) && this.gridObj5) {
      args.rows.map((idx) => {
        const item = this.gridObj5.getDataItem(idx);
        controls.so_po.setValue(item.so_nbr || "");
        controls.so_cust.setValue(item.so_cust)
        controls.so_curr.setValue(item.so_curr)
        controls.so_cr_terms.setValue(item.so_cr_terms)
        controls.so_taxable.setValue(item.so_taxable)
    controls.so_bill.setValue(item.so_bill)

        this.quoteService.findByOne({ so_nbr: item.so_nbr }).subscribe(
          (res: any) => {
            const { quoteOrder, details } = res.data;
            console.log(details)
            ...
            this.customersService.getBy({ cm_addr: item.so_cust }).subscribe(
              (res: any) => {
                //console.log(res);
                ;
                this.customer = res.data;
...
console.log(updateItem.sod_part)
      ...
      this.itemsService.getByOne({pt_part: updateItem.sod_part }).subscribe((resp:any)=>{
...
prepareGridlocdet() {
  ...
  this.locationDetailService
    .getBy({ ld_site:  updateItem.sod_site , ld_loc:  updateItem.sod_loc, ld_part:  updateItem.sod_part,ld_ref: null })
...
printpdf(nbr) {
  ...
  if (this.customer.address.ad_line1 != null) {

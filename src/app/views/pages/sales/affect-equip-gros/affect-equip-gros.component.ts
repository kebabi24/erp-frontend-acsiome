...
@Component({
  selector: 'kt-affect-equip-gros',
  templateUrl: './affect-equip-gros.component.html',
  styleUrls: ['./affect-equip-gros.component.scss']
})
export class AffectEquipGrosComponent implements OnInit {
  aeForm: FormGroup;
  ...
  createForm() {
    ...
    this.aeForm = this.aeFB.group({
      ...
      ae_eqp_nbr : [1, Validators.required],
  ...
  handleSelectedRowsChanged2(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObj2) {
      args.rows.map((idx) => {
        ...
        controls.ae_cust.setValue(item.cm_addr || "");
  ...
  prepareGrid2() {
    ...
    const controls = this.aeForm.controls;
    this.customerService.getByAll({}).subscribe((response: any) => (this.customers = response.data));
  }
  open2(content) {
    this.prepareGrid2();
...
onchangeeqp() {
  ...
  controls.ae_amt.setValue(Number(controls.ae_amt_un.value )* Number(controls.ae_eqp_nbr.value)  )
}
onchangenbr() {
  const controls = this.aeForm.controls
...
printpdf() {
  ...
  doc.text("Bon de Sortie : " + this.aenbr , 70, initialY + 5);

...
@Component({
  selector: 'kt-create-so-plq',
  templateUrl: './create-so-plq.component.html',
  styleUrls: ['./create-so-plq.component.scss']
})
export class CreateSoPlqComponent implements OnInit {
        ...
        onCellChange: (e: Event, args: OnEventArgs) => {
          ...
          this.itemsService.getByOne({ pt_part: args.dataContext.sod_part }).subscribe((resp: any) => {
            if (resp.data) {
              ...
              this.gridService.updateItemById(args.dataContext.id, { ...args.dataContext, desc: resp.data.pt_desc1, sod_site: resp.data.pt_site, sod_loc: resp.data.pt_loc, sod_um: resp.data.pt_um, sod_um_conv: 1,  sod_disc_pct: 0, sod_type: this.type, sod_tax_code: resp.data.pt_taxc, sod_taxc: resp.data.taxe.tx2_tax_pct, sod_taxable: this.taxable });
            } else {
  ...
  addSo(_so: any, detail: any) {
    ...
    this.saleOrderService.add({ saleOrder: _so, saleOrderDetail: detail }).subscribe(
      ...
      () => {
        ...
        this.reset()
        this.router.navigateByUrl("/sales/create-so-plq");
      }
    );
  }
  onChangeOC() {
  ...
  calculatprice(){
    ...
    for (let d of this.dataset){
      qty = qty + Number(d.sod_qty_ord)
    }
    this.priceListQuantityService.getByOne({qty}).subscribe((response: any) => {
      if (!response.data) {
        alert("Liste Prix  nexiste pas");
        // controls.so_channel.setValue(null);
        // console.log(response.data.length);
        // document.getElementById("so_channel").focus();
      } else {
        ...
        for(let data of this.dataset) {
          let index = det.findIndex((element) => {
            return element.plqd_part === data.sod_part;
          });
          data.sod_price = det[index].plqd_salesprice
...
this.data=[]
      ...
      let i =0
      if(this.dataset.length == 0) { i = 0} else { i = Math.max(...ids) + 1}
      console.log(i)
      for(let d of this.selectedItem) {
        let index = this.dataset.findIndex((element) => {
          return element.sod_part === d.pt_part;
        });
        if (index < 0) {
        this.dataset.push({
          id: i,
          ...
          sod_site : d.pt_site,
          sod_loc : d.pt_loc,
          sod_taxable : d.pt_taxable,
          sod_tax_code : d.pt_taxc,
          sod_taxc : d.taxe.tx2_tax_pct,
          sod_type : 'M',
         // sod_price : d.pt_price,
          sod_disc_pct : 0,
      })
...
// this.updateItemMetadata(this.dataView.getItemMetadata);
 ...
 this.modalService.dismissAll()
  ...
  printpdf(nbr) {
    ...
    doc.setFontSize(12);
    doc.text("Commande N° : " + nbr, 87, 50);
    doc.setFontSize(10);
    console.log(this.customer.address.ad_misc2_id);
    doc.text("Code Client : " + this.customer.cm_addr, 20, 55);
    doc.text("Nom             : " + this.customer.address.ad_name, 20, 60);
    doc.text("Adresse       : " + this.customer.address.ad_line1, 20, 65);
    if (this.customer.address.ad_misc2_id != null) {
      doc.text("MF          : " + this.customer.address.ad_misc2_id, 20, 70);
    }
    if (this.customer.address.ad_gst_id != null) {
      doc.text("RC          : " + this.customer.address.ad_gst_id, 20, 75);
    }
    if (this.customer.address.ad_pst_id) {
      doc.text("AI            : " + this.customer.address.ad_pst_id, 20, 80);
    }
    if (this.customer.address.ad_misc1_id != null) {
      doc.text("NIS         : " + this.customer.address.ad_misc1_id, 20, 85);
    }

    doc.line(10, 90, 200, 90);
    doc.line(10, 95, 200, 95);
    doc.line(10, 90, 10, 95);
    doc.text("LN", 12.5, 93.5);
    doc.line(20, 90, 20, 95);
    doc.text("Code Article", 25, 93.5);
    doc.line(45, 90, 45, 95);
    doc.text("Désignation", 67.5, 93.5);
    doc.line(100, 90, 100, 95);
    doc.text("QTE", 107, 93.5);
    doc.line(120, 90, 120, 95);
    doc.text("UM", 123, 93.5);
    doc.line(130, 90, 130, 95);
    doc.text("PU", 138, 93.5);
    doc.line(150, 90, 150, 95);
    doc.text("TVA", 152, 93.5);
    doc.line(160, 90, 160, 95);
    doc.text("REM", 162, 93.5);
    doc.line(170, 90, 170, 95);
    doc.text("THT", 181, 93.5);
    doc.line(200, 90, 200, 95);

...
error = false;
  ...
  datasetso: [];
  columnDefinitionsso: Column[] = [];
  gridOptionsso: GridOption = {};
  gridObjso: any;
  angularGridso: AngularGridInstance;
  ...
  //    so_category: [this.saleOrder.so_category , Validators.required],
    ...
    const controls = this.soForm.controls;
this.customersService.getBy({ cm_addr : "CLT-0001" }).subscribe(
      (res: any) => {
        console.log(res);
        const { data } = res;
        ...
        if (!data) {
          ...
          controls.so_cust.setValue(this.biller.cm_addr)
          controls.name.setValue(this.biller.address.ad_name)
          controls.so_cr_terms.setValue(this.biller.cm_cr_terms)
  ...
  nbr: [null],
      ...
      ht_amt: [{value: 0.00 , disabled: true}],
      tax_amt: [{value: 0.00 , disabled: true}],
      trl1_amt: [{value: 0.00 , disabled: true}],
      total_amt: [{value: 0.00 , disabled: true}],
    });
  ...
  createuserForm() {
   ...
   // const date = new Date;
    ...
    this.userForm = this.sodFB.group({
  //    so__chr01: [this.saleOrder.so__chr01],
      user: [null],
      password: [null],
    });
    ...
    this.saleOrderService
      ...
      .subscribe(
        ...
        () => {
          ...
          this.reset();
          this.loadingSubject.next(true);
          this.router.navigateByUrl("/sales/sales-box");
...
changeTax(){
  ...
  addpart(){
    ...
    const controls = this.soForm.controls;
    console.log(controls.barecode.value)
    this.itemsService.getByOne({ pt_article: controls.barecode.value }).subscribe((resp:any)=>{
          ...
          (response: any) => {
            ...
            //  this.gridService.updateItemById(args.dataContext.id,{...args.dataContext , desc: resp.data.pt_desc1 , sod_site:resp.data.pt_site, sod_loc: resp.data.pt_loc, sod_serial: null,
              ...
              this.gridService.addItem(
                ...
                sod_status : this.stat,
                sod_expire : this.lddet.ld_expire,
 ...
 doc.setFontSize(8);
  ...
  prepareGridso() {
    ...
    `
        ...
        onCellClick: (e: Event, args: OnEventArgs) => {
            const soid = args.dataContext.soid
            console.log(soid)
            this.sodid = soid
           // this.router.navigateByUrl(`/sales/edit-so/${soid}`)
        //   this.opensod(contentsod,soid)
           let element: HTMLElement = document.getElementById(
            "openUsersGrid"
          ) as HTMLElement;
          element.click();
  ...
  // fill the dataset with your data
  this.datasetso = []
  this.saleOrderService.getByAllSo({so_ord_date: new Date}).subscribe(
      (response: any) =>  ( this.datasetso = response.data),
  ...
  handleSelectedRowsChangedso(e, args) {
    const controls = this.soForm.controls;
    if (Array.isArray(args.rows) && this.gridObj2) {
      args.rows.map((idx) => {
        const item = this.gridObj2.getDataItem(idx);
  ...
  this.columnDefinitionssod = [
    {
      ...
      onCellClick: (e: Event, args: OnEventArgs) => {
        ...
        if (confirm("Êtes-vous sûr d'annuler cette ligne?")) {
          //this.angularGridsod.gridService.deleteItem(args.dataContext);
        if(args.dataContext.sod_confirm != true) {
          this.gridServicesod.updateItemById(args.dataContext.id,{...args.dataContext , sod_confirm: true , soup:true});
          this.calculatesodtot()
        } else {
          alert("produit deja annule")
        }
        }
      },
    },
...
// fill the dataset with your data
this.sods = []
console.log(this.sodid)
const controls = this.sodForm.controls
this.saleOrderService.getBy({id: this.sodid}).subscribe(
    (response: any) =>   { 
      this.sods = response.data.details
      console.log("hnahna",response.data)
      controls.nbr.setValue(response.data.saleOrder.so_nbr)
      controls.ht_amt.setValue(response.data.saleOrder.so_amt)
      controls.tax_amt.setValue(response.data.saleOrder.so_tax_amt)
      controls.trl1_amt.setValue(response.data.saleOrder.so_trl1_amt)
      controls.total_amt.setValue(Number(response.data.saleOrder.so_amt) + Number(response.data.saleOrder.so_tax_amt) + Number(response.data.saleOrder.so_trl1_amt) )
    }
...
handleSelectedRowsChangedsod(e, args) {
  const controls = this.soForm.controls;
  if (Array.isArray(args.rows) && this.gridObj2) {
    args.rows.map((idx) => {
      const item = this.gridObj2.getDataItem(idx);
...
angularGridReadysod(angularGrid: AngularGridInstance) {
  ...
  this.gridObjsod = (angularGrid && angularGrid.slickGrid) || {};
...
calculatesodtot(){
  const controls = this.sodForm.controls 
   ...
   let stimbre = 0
   let sttc = 0
   for (var i = 0; i < this.sods.length; i++) {
     if (this.sods[i].sod_confirm == false) {
     console.log("here here " ,this.sods[i]  )
     stht += round((this.sods[i].sod_price * ((100 - this.sods[i].sod_disc_pct) / 100 ) *  this.sods[i].sod_qty_ord),2)
     stva += round((this.sods[i].sod_price * ((100 - this.sods[i].sod_disc_pct) / 100 ) *  this.sods[i].sod_qty_ord) * (this.sods[i].sod_taxc ? this.sods[i].sod_taxc / 100 : 0),2)
   ...
   if(controls.trl1_amt.value != 0) { stimbre = round((stht + stva) / 100,2);
    if (stimbre > 2500) { stimbre = 2500} } 

 sttc = round(stht + stva + stimbre,2)

controls.ht_amt.setValue(stht.toFixed(2));
controls.tax_amt.setValue(stva.toFixed(2));
controls.trl1_amt.setValue(stimbre.toFixed(2));
controls.total_amt.setValue(sttc.toFixed(2));

}

onSubmituser(){
const controls = this.userForm.controls
if (controls.user.value == "admin" && controls.password.value == "admin") {
...
alert("Vous n'etes pas autoriser a modifié les Bons")
document.getElementById("user").focus();
controls.user.setValue(null)
controls.password.setValue(null)
...
onUpdateSo(){
  ...
  const controls = this.sodForm.controls
  let so = {so_amt: Number(controls.ht_amt.value), so_tax_amt: Number(controls.tax_amt.value), so_trl1_amt: Number(controls.trl1_amt.value)}
  let soddetails = this.sods
  
  this.saleOrderService.updateSod({ so, soddetails }, this.sodid)
          .subscribe( //(res) => {

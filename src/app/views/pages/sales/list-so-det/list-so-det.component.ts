...
@Component({
  selector: 'kt-list-so-det',
  templateUrl: './list-so-det.component.html',
  styleUrls: ['./list-so-det.component.scss']
})
export class ListSoDetComponent implements OnInit {
...
doc.line(200, 100, 200, 105);
  ...
  doc.text(String(Number(this.so.so_amt + this.so.so_tax_amt +this.so.so_trl1_amt ).toFixed(2)), 198, i + 33, { align: "right" });

  doc.setFontSize(12);
  let mt = NumberToLetters(Number(this.so.so_amt + this.so.so_tax_amt +this.so.so_trl1_amt ).toFixed(2), "Dinars Algérien");

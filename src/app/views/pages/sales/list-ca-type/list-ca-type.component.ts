...
@Component({
  selector: 'kt-list-ca-type',
  templateUrl: './list-ca-type.component.html',
  styleUrls: ['./list-ca-type.component.scss']
})
export class ListCaTypeComponent implements OnInit {
  ...
  solist() {
    ...
    let obj= {date,date1}
    this.mobileSettingsService.getSalesRoleType(obj).subscribe(
      (response: any) => {   
        this.mvdataset = response.data
...
data.reduce(function(res, value) {
  if (!res[value.code_cmmt]) {
    res[value.code_cmmt] = { code_cmmt: value.code_cmmt, qty: 0,prix:0 };
    result.push(res[value.code_cmmt])
  }
  res[value.code_cmmt].qty += value.quantity;
  res[value.code_cmmt].prix += value.price;
...
//console.log(result)
   ...
   /*teste*/
    ...
    // doc.text('This is client-side Javascript, pumping out a PDF.', 20, 30);
     ...
     doc.setFontSize(14);
     doc.text("Etat Chiffre D'Affaire par Type  : " , 70, 40);
     doc.setFontSize(12);
     
     doc.text("Date Début : " + date, 40, 55);
     doc.text("Date Fin      : " + date1, 120, 55);
     
     doc.setFontSize(10);
          doc.line(30, 65, 200, 65);
          doc.line(30, 70, 200, 70);
          doc.line(30, 65, 30, 70);
          doc.text("Role", 42, 68.5);
          doc.line(70, 65, 70, 70);
          doc.text("Type", 72, 68.5);
          doc.line(100, 65, 100, 70);
          doc.text("Quantité", 110, 68.5);
          doc.line(150, 65, 150, 70);
          doc.text("Montant", 160, 68.5);
          doc.line(200, 65, 200, 70);
     ...
     for (let j = 0; j < data.length; j++) {
       let mts =  String(  Number(data[j].price).toLocaleString("en-US", {
    ...
    //   console.log(mts)
      ...
      // console.log(mnsolde)
       if (j % 38 == 0 && j != 0) {
         ...
         if (j== 0) {doc.text(data[j].role, 35, i - 1);} else {
          if(data[j].role == data[j-1].role ) {doc.text(" ", 35, i - 1)} else {
            doc.text(data[j].role, 35, i - 1);}
         }
         doc.line(70, i - 5, 70, i);
         doc.text(data[j].code_cmmt, 75, i - 1);
         doc.line(100, i - 5, 100, i);
         doc.text(String(data[j].quantity), 148, i - 1,{ align: "right" });
         doc.line(150, i - 5, 150, i);
         doc.text(mnt, 198, i - 1,{ align: "right" });
         doc.line(200, i - 5, 200, i);
        console.log(j,data.length)
         if(j   < data.length - 1) {
          if(data[j+1].role == data[j].role ) { doc.line(70, i, 200, i)} else {
            doc.line(30, i, 200, i);} }
            else {
              doc.line(30, i, 200, i)
            }
          ...
          //doc.line(30, i, 200, i);
         i = i + 5;
         total = total + Number(data[j].price)
         qt = qt + Number(data[j].quantity)
        }
        doc.line(30, i-5, 200, i-5);
    ...
    //   console.log(mts)
       ...
       let ttca = replaceAll(ttecq,","," ")

        
let tqt =  Number(qt)
        let ttes =  String(  Number(tqt).toLocaleString("en-US", {
         minimumFractionDigits: 2,
         maximumFractionDigits: 2,
       }))
    //   console.log(mts)
       ...
       let ttqt = replaceAll(ttes,","," ")

     
        doc.line(70, i - 5, 70, i);
        
         // doc.line(40, i - 5, 40, i);
         doc.text("Total ", 75, i - 1);
         doc.line(100, i - 5, 100, i);
         doc.text(ttqt, 148, i - 1,{ align: "right" });
         doc.line(150, i - 5, 150, i); 
         doc.text(ttca, 198, i - 1,{ align: "right" });
         doc.line(200, i - 5, 200, i);
         doc.line(70, i, 200, i);
         i = i + 5;
         ...
         for(let data of result) {
          ...
          let tca =  String(  Number(data.prix).toLocaleString("en-US", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }))
       //   console.log(mts)
          ...
          let ttca = replaceAll(tca,","," ")
          let tq =  String(  Number(data.qty).toLocaleString("en-US", {
            minimumFractionDigits: 2,
            maximumFractionDigits: 2,
          }))
       //   console.log(mts)
          ...
          let ttq = replaceAll(tq,","," ")
          doc.line(70, i - 5, 70, i);
          doc.text(data.code_cmmt, 75, i - 1);
          doc.line(100, i - 5, 100, i);
          doc.text(ttq, 148, i - 1,{ align: "right" });
          doc.line(150, i - 5, 150, i);
          doc.text(ttca, 198, i - 1,{ align: "right" });
          doc.line(200, i - 5, 200, i);
          doc.line(70, i, 200, i);
      i = i + 5
        ...
        //  //doc.line(40, i - 5, 40, i);
        //  doc.text("Total QTE", 35, i - 1);
        //  doc.line(70, i - 5, 70, i);
        //  doc.text(ttqt, 108, i - 1,{ align: "right" });

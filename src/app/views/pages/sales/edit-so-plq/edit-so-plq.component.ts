...
@Component({
  selector: 'kt-edit-so-plq',
  templateUrl: './edit-so-plq.component.html',
  styleUrls: ['./edit-so-plq.component.scss']
})
export class EditSoPlqComponent implements OnInit {
  saleOrder: SaleOrder;
  soForm: FormGroup;
  totForm: FormGroup;
  hasFormErrors = false;
...
mt:any;
  ...
  date1: any;
  date2: any;
  title: String = 'Modifier Commande - '
  soEdit: any
  cfg : any;
  curr
  domain
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.params.subscribe((params) => {
        ...
        this.saleOrderService.getOne(id).subscribe((response: any)=>{
          ...
          this.ov_amt = this.soEdit.so_prepaid
this.soEdit.so_prepaid = Number(this.soEdit.so_amt) + Number(this.soEdit.so_tax_amt) + Number(this.soEdit.so_trl3_amt) - Number(this.soEdit.so_prepaid)
this.solde = Number(this.soEdit.so_amt) + Number(this.soEdit.so_tax_amt) + Number(this.soEdit.so_trl3_amt) - Number(this.soEdit.so_prepaid)
          this.nom     = this.soEdit.so_cust
          this.customersService.getBy({ cm_addr: this.soEdit.so_cust }).subscribe(
            ...
            this.deviseService.getBy({ cu_curr: this.soEdit.so_curr }).subscribe(
              (respo: any) => {
                console.log(respo);
          ...
          console.log(this.soEdit)
            ...
            this.site = response.data.details[0].sod_site
            this.loctype = response.data.details[0].sod_part
...
initCode() {
  ...
  //    so__chr01: [this.saleOrder.so__chr01],
      ...
      so_bank: [this.soEdit.so_bank],
      // so_prepaid: [this.soEdit.so_prepaid],
  ...
  //  _so.so_prepaid = Number(controls.so_prepaid.value) + Number(this.ov_amt);
  //  _so.dec01   = Number(controls.so_prepaid.value) 
        ...
        () => {
         ...
         // if(controls.print.value == true) printSO(this.customer, this.dataset, so);
          if(controls.print.value == true) {
            ...
            // }
            // else{this.nom = 'FNPOS'}
            this.printpdf(this.soEdit.so_nbr)} 
         this.router.navigateByUrl("/sales/list-so-edit");
        }
      );
  }
  onChangeOC() {
...
calculatetot(){
         const controls = this.totForm.controls 
          ...
          for (var i = 0; i < this.dataset.length; i++) {
            ...
            if(controlsso.so_cr_terms.value == "ES") { if((tht + tva) <= 30000){timbre = round((tht + tva) / 100, 2)}
            else{
...
printpdf(nbr) {
  ...
  const due =  Number(String(this.date1.getDate())).toString().padStart(2, '0') + "/" + Number(String(this.date1.getMonth() + 1)).toString().padStart(2, '0')+ "/" + (String(this.date1.getFullYear()));  
  var doc = new jsPDF();
   var img = new Image();
...
doc.setFontSize(10);
    ...
    if (this.dataset[j].item.pt_desc1.length > 35) {
      let desc1 = this.dataset[j].pt_desc1.substring(35)
      ...
      doc.text( String(Number(this.dataset[j].sod_qty_ord).toFixed(2)), 118 , i  - 1 , { align: 'right' });

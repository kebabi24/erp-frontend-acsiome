...
import { SaleOrderService, AddressService, QuoteService, SequenceService, CustomerService, UsersService, ItemService, SaleOrder, TaxeService, DeviseService, CodeService, SiteService, LocationService, MesureService, PricelistService, LocationDetailService, InventoryStatusService, InvoiceOrderTemp, InvoiceOrderTempService, ProjectService, printSO, ConfigService, PayMethService, PsService } from "../../../../core/erp";
...
export class CreateProjectInvoiceComponent implements OnInit {
  ...
  constructor(
    ...
    this.configService.getOne(1).subscribe((resp: any) => {
      ...
      if (this.cfg) {
        ...
        this.codeService.getBy({ code_fldname: "cm_cr_terms" }).subscribe((response: any) => (this.ith_cr_terms = response.data));
        console.log(this.ith_cr_terms);
      }
    });
  ...
  generateFacturePDF() {
    ...
    console.log(controls.ith_inv_date.value);
    ...
    setTimeout(function () {
      ...
      const value = controls.ith_type.value;
      console.log(value);
      ...
      html2canvas(factureContent, { scale: scale }).then((canvas) => {
  ...
  prepareIth(): any {
    ...
    _ith.ith_inv_date = controls.ith_inv_date.value ? `${controls.ith_inv_date.value.year}/${controls.ith_inv_date.value.month}/${controls.ith_inv_date.value.day}` : null;
    _ith.ith_due_date = controls.ith_due_date.value ? `${controls.ith_due_date.value.year}/${controls.ith_due_date.value.month}/${controls.ith_due_date.value.day}` : null;
    if (controls.ith_taxable.value == null || controls.ith_taxable.value == "") {
      _ith.ith_taxable = false;
    } else {
      _ith.ith_taxable = controls.ith_taxable.value;
    }

    _ith.ith_po = controls.ith_po.value;
    _ith.ith_type = controls.ith_type.value;
    _ith.ith_disc_glb = controls.ith_disc_glb.value;
    _ith.ith_transport = controls.ith_transport.value;
    _ith.ith_rt_gara = controls.ith_rt_gara.value;
  ...
  onChangeBill() {
    ...
    const date = new Date();
    this.date = controls.ith_inv_date.value ? `${controls.ith_inv_date.value.year}/${controls.ith_inv_date.value.month}/${controls.ith_inv_date.value.day}` : `${date.getFullYear()}-${date.getMonth() + 1}-${date.getDate()}`;

    //this.dataset = [];
    this.customersService.getBy({ cm_addr }).subscribe((res: any) => {
      console.log(res);
      const { data } = res;
  ...
  getMonthName(month) {
    const MonthList = ["Janvier", "Février", "Mars", "Avril", "Mai", "Juin", "Juillet", "Aout", "Septembre", "Octobre", "Décembre"];
  ...
  changeRateCurr() {
    ...
    console.log("dateeeeeeeeeeeeeeeeee", controls.ith_inv_date.value);
    this.invoiceYear = controls.ith_inv_date.value.year;
    this.invoiceMonth = this.getMonthName(controls.ith_inv_date.value.month - 1);
    this.invoiceDate = controls.ith_inv_date.value ? `${controls.ith_inv_date.value.year}/${controls.ith_inv_date.value.month}/${controls.ith_inv_date.value.day}` : null;
    ...
    this.customersService.getBy({ cm_addr }).subscribe(
      (res: any) => {
        ...
        if (!data) {
          ...
          this.addressService.getBy({ ad_addr: data.cm_bill }).subscribe((resadr: any) => {
  ...
  handleSelectedRowsChanged2(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObj2) {
      args.rows.map((idx) => {
        ...
        //   this.dataset = [];
        this.customersService.getBy({ cm_addr: item.cm_bill }).subscribe((ressa: any) => {
          console.log(ressa);
          ...
          this.deviseService.getBy({ cu_curr: ressa.data.cm_curr }).subscribe((res: any) => {
            console.log(res);
            const { data } = res;
            if (data) {
              this.curr = data;
  ...
  prepareGrid2() {
    ...
    this.customersService.getAll().subscribe((response: any) => (this.customers = response.data));
  }
  open2(content) {
    this.prepareGrid2();
  ...
  handleSelectedRowsChanged4(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObj4) {
      args.rows.map((idx) => {
        ...
        if (controls.ith_taxable.value == false) {
          this.taxable = false;
        } else {
          this.taxable = item.pt_taxable;
        }

        this.locationDetailService.getByOne({ ld_site: item.pt_site, ld_loc: item.pt_loc, ld_part: item.pt_part, ld_lot: null }).subscribe((response: any) => {
  ...
  handleSelectedRowsChanged5(e, args) {
    ...
    */
        ...
        this.saleOrderService.getBy({ so_po: item.pm_code }).subscribe((res: any) => {
          const { saleOrder, details } = res.data;
          console.log(details);
          controls.ith_curr.setValue(saleOrder.so_curr);
          controls.ith_nbr.setValue(saleOrder.so_nbr);
          ...
          this.customersService.getBy({ cm_addr: item.pm_cust }).subscribe((res: any) => {
            //console.log(res);
            this.customer = res.data;
  ...
  prepareGrid5() {
    ...
    // fill the dataset with your data
    this.projectService.getAll().subscribe((response: any) => (this.quotes = response.data));
  }
  open5(content) {
    this.prepareGrid5();
    this.modalService.open(content, { size: "lg" });
  ...
  prepareGridloc() {
    ...
    // fill the dataset with your data
    this.locationService.getBy({ loc_site: updateItem.itdh_site }).subscribe((response: any) => (this.dataloc = response.data));
  }
  openloc(contentloc) {
    this.prepareGridloc();
  ...
  prepareGrid1() {
    ...
    this.sequencesService.getBy({ seq_type: "IV", seq_profile: this.user.usrd_profile }).subscribe((response: any) => (this.sequences = response.data));
  }
  open(content) {
    this.prepareGrid1();
    this.modalService.open(content, { size: "lg" });
  ...
  prepareGridlocdet() {
    ...
    // fill the dataset with your data
    this.locationDetailService.getBy({ ld_site: updateItem.itdh_site, ld_loc: updateItem.itdh_loc, ld_part: updateItem.itdh_part }).subscribe((response: any) => (this.datalocdet = response.data));
  }
  ...
  prepareGridbill() {
    ...
    // fill the dataset with your data
    this.customersService.getAll().subscribe((response: any) => (this.bills = response.data));
  }
  openbill(content) {
  ...
  printpdf(nbr) {
    ...
    for (let j = 0; j < this.dataset.length; j++) {
      ...
      if (this.dataset[j].desc.length > 35) {
        ...
        doc.text(String((this.dataset[j].itdh_price * ((100 - this.dataset[j].itdh_disc_pct) / 100) * this.dataset[j].itdh_qty_inv).toFixed(2)), 198, i - 1, { align: "right" });
        ...
        if (this.dataset[j].itdh__chr03 != null) {
          if (this.dataset[j].itdh__chr03.length > 42) {
            ...
            if (ind == -1) {
              ind = this.dataset[j].itdh__chr03.length - 42;
            }

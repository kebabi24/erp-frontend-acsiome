...
@Component({
  selector: 'kt-undo-invoice',
  templateUrl: './undo-invoice.component.html',
  styleUrls: ['./undo-invoice.component.scss']
})
export class UndoInvoiceComponent implements OnInit {
  ...
  createForm() {
    ...
    //    so__chr01: [this.invoiceOrder.ih__chr01],
    ih_category : [this.invoiceOrder.ith_category,Validators.required],
        ih_inv_nbr: [{value :this.invoiceOrder.ith_inv_nbr , disabled:true} ,Validators.required],
        ih_cust: [{value: this.invoiceOrder.ith_cust , disabled: true}],
        name: [{value:"", disabled: true}],
        
        ih_bill: [{value: this.invoiceOrder.ith_bill , disabled: true}],
        namebill: [{value:"", disabled: true}],
        
        ih_inv_date_a: [{value:null, disabled:true}],
        ih_inv_date: [{
        ...
        ih_taxable: [{value: this.invoiceOrder.ith_taxable, disabled: true}],
       
        // ih_po: [{value:this.invoiceOrder.ih_po, disabled: true}],
        ih_rmks: [this.invoiceOrder.ith_rmks ],
        ih_curr: [{value:this.invoiceOrder.ith_curr, disabled: true}],
        ih_ex_rate: [{value:this.invoiceOrder.ith_ex_rate, disabled: true}],
        ih_ex_rate2: [{value:this.invoiceOrder.ith_ex_rate2, disabled: true}],
        ih_cr_terms: [{value:this.invoiceOrder.ith_cr_terms, disabled:true}, Validators.required],
        print:[true],
        // duplicata:["DUPLICATA"]
      });
  ...
  prepareIh(): any {
    ...
    _ih.ith_category =  controls.ih_category.value
    _ih.ith_cust = controls.ih_cust.value;
    _ih.ith_bill = controls.ih_bill.value;
    // _ih.ih_nbr = controls.ih_nbr.value;
    
    _ih.ith_inv_date = controls.ih_inv_date.value
      ? `${controls.ih_inv_date.value.year}/${controls.ih_inv_date.value.month}/${controls.ih_inv_date.value.day}`
      : null;
    
      if (controls.ih_taxable.value == null || controls.ih_taxable.value == "" ) { _ih.ith_taxable = false} else { _ih.ith_taxable = controls.ih_taxable.value}
    
   
    _ih.ith_rmks = controls.ih_rmks.value;
    
    _ih.ith_po = controls.ih_inv_nbr.value;
    _ih.ith_curr = controls.ih_curr.value;
    _ih.ith_ex_rate = controls.ih_ex_rate.value;
    _ih.ith_ex_rate2 = controls.ih_ex_rate2.value;
    _ih.ith_cr_terms = controls.ih_cr_terms.value;
  ...
  addIh(_ih: any, detail: any) {
    ...
    this.invoiceOrderTempService
      .addAv({ invoiceOrder: _ih, invoiceOrderDetail: detail })
      .subscribe(
  ...
  if(controls.print.value == true) this.printpdf(ih) //printIH(this.customer, iharray, ih,this.curr);
          ...
          this.reset()
          this.router.navigateByUrl("/sales/undo-invoice");
          this.reset()
        }
      );
  }
      ...
      args.rows.map((idx) => {
        ...
        this.invoiceOrderService.getByOne({ ih_inv_nbr }).subscribe(
          (res: any) => {
            ...
            controlss.tht.setValue(-Number(this.soServer.ih_amt))
            controlss.tva.setValue(-Number(this.soServer.ih_tax_amt))
            controlss.timbre.setValue(-Number(this.soServer.ih_trl1_amt))
            controlss.ttc.setValue(-Number(this.soServer.ih_tot_amt))

            controls.ih_inv_date_a.setValue(String(this.soServer.ih_inv_date)|| "")
                  ...
                  this.gridServiceih.addItem(
                    {
                      ...
                      cmvid: "",
                      desc: detail.item.pt_desc1,
                      idh_qty_inv: -Number(detail.idh_qty_inv ),
                      idh_um: detail.idh_um,
                      idh_um_conv: detail.idh_um_conv,
                      idh_type: detail.idh_type,
                      idh_price: detail.idh_price,
...
printpdf(nbr) {
 ...
 ? `${controlss.ih_inv_date.value.day}/${controlss.ih_inv_date.value.month}/${controlss.ih_inv_date.value.year}`
 : " ";
...
console.log("this.iharray[j].desc",this.iharray)
    if (this.iharray[j].desc.length > 35) {
      ...
      doc.text(String(-Number(this.iharray[j].itdh_price *
 ...
 doc.setFontSize(8);
    let mt = NumberToLetters(
      ...
      if (mt.length > 95) {
        ...
        doc.text( "Arretée le présent Avoir a la somme de :" + mt1  , 20, i + 53)
        doc.text(  mt2  , 20, i + 60)
      } else {
        doc.text( "Arretée le présente Avoir a la somme de :" + mt  , 20, i + 53)

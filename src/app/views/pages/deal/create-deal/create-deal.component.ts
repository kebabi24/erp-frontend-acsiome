...
import { NgbDropdownConfig, NgbTabChangeEvent, NgbTabsetConfig, NgbModal } from "@ng-bootstrap/ng-bootstrap";
...
export class CreateDealComponent implements OnInit {
  ...
  constructor(config: NgbDropdownConfig, private dealFB: FormBuilder, private activatedRoute: ActivatedRoute, private router: Router, public dialog: MatDialog, private layoutUtilsService: LayoutUtilsService, private dealService: DealService, private codeService: CodeService, private customerService: CustomerService, private modalService: NgbModal) {
    config.autoClose = true;
    this.codeService.getBy({ code_fldname: "deal_pay_meth" }).subscribe((response: any) => (this.deal_pay_meth = response.data));
    this.codeService.getBy({ code_fldname: "deal_inv_meth" }).subscribe((response: any) => (this.deal_inv_meth = response.data));
    this.codeService.getBy({ code_fldname: "deal_status" }).subscribe((response: any) => (this.deal_status = response.data));
  }
  ...
  uploadFile(event) {
    if (event.target.files.length > 0) {
      ...
      this.dealForm.get("attach").setValue(file);
    }
  }
  ...
  prepareDeal(): Deal {
    ...
    _deal.deal_end_date = controls.deal_end_date.value ? `${controls.deal_end_date.value.year}/${controls.deal_end_date.value.month}/${controls.deal_end_date.value.day}` : null;
  ...
  addDeal(_deal: Deal) {
    ...
    // console.log(this.dealForm.get("attach").value);
    formData.append("file", this.dealForm.get("attach").value);
    formData.append("data", JSON.stringify(_deal));
    this.loadingSubject.next(true);
    this.dealService.add(formData).subscribe(

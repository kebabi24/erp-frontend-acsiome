...
export class CreateUserMobileComponent implements OnInit {
  ...
  message: any;
  profile_code: String;
  constructor(config: NgbDropdownConfig, private userFB: FormBuilder, private activatedRoute: ActivatedRoute, private router: Router, public dialog: MatDialog, private layoutUtilsService: LayoutUtilsService, private userMobileService: UsersMobileService, private modalService: NgbModal) {
  ...
  onChangeCode() {
    ...
    this.userMobileService.getByOne({ user_mobile_code: controls.user_mobile_code.value }).subscribe((res: any) => {
  ...
  prepareGrid() {
    ...
    this.userMobileService.getAllProfiles().subscribe((response: any) => (this.profiles = response.data));
  }

...
@Component({ 
  selector: 'kt-rct-tr-globalprint',
  templateUrl: './rct-tr-globalprint.component.html',
  styleUrls: ['./rct-tr-globalprint.component.scss']
})


export class RctTrGlobalprintComponent implements OnInit {
     ...
     // console.log(this.site)  
      ...
      addIt( detail: any, it, nlot) {
        ...
        this.inventoryTransactionService
          ...
          if (controls.print.value == true) {this.printpdf(nlot,nlot)};
            this.router.navigateByUrl("/");
            this.loadingSubject.next(false);
            }
          );
...
onactive(){
  ...
  this.inventoryTransactionService.getByRefs({ tr_type,date1,date2,time1,time2 }).subscribe(
    ...
    (error) => {
      this.message = `Transfert n'existe pas`;
      this.hasFormErrors = true;
    },
    () => {}
  );
...
open5(content) {
  ...
  if(this.provider != null){name = this.provider.ad_name, code=this.provider.ad_addr, addresse = this.provider.ad_line1}
   ...
   // doc.text('This is client-side Javascript, pumping out a PDF.', 20, 30);
    var img = new Image()
    // img.src = "./assets/media/logos/rct-tr-globalprint.png";
    ...
    doc.setFontSize(12);
    doc.text("Liste des Transferts DU: " + nbr + " AU: " + nbr2, 70, 45);
    ...
    doc.text("NOMBRE DE BIG BAG   " + String(this.dataset.length) + "  ,TOTAL POID:    " + String(Number(total)), 40, i + 12, { align: "left" });
...
/*
  ...
  onChangeCC() {
    ...
    this.dataset = [];
        this.inventoryTransactionService.getByNbr({ tr_lot: controls.tr_lot.value, tr_type:"RCT-TR" }).subscribe(
          (res: any) => {
            console.log(res)
           this.dataset = res.data;
           if (this.dataset.length > 0) {
            ...
            this.inventoryTransactionService.getByRef({ tr_lot: controls.tr_lot.value, tr_type:"RCT-TR" }).subscribe(
              (resp: any) => {
                console.log(resp)

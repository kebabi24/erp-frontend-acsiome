...
@Component({
  selector: 'kt-po-receipt-det',
  templateUrl: './po-receipt-det.component.html',
  styleUrls: ['./po-receipt-det.component.scss']
})
export class PoReceiptDetComponent implements OnInit {
...
addIt(detail: any, pr, prhnbr) {
  ...
  const controls = this.prhForm.controls;
  let poNbr = 0;
  this.purchaseReceiveService.addCabDet({ detail, pr, prhnbr }).subscribe(
...
prepareGrid5() {
  ...
  // fill the dataset with your data
  this.purchaseOrderService.getByAll({}).subscribe((response: any) => {
    console.log(response.data);

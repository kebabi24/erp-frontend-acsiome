...
@Component({
  selector: 'kt-po-receip-cab-id-lab',
  templateUrl: './po-receip-cab-id-lab.component.html',
  styleUrls: ['./po-receip-cab-id-lab.component.scss']
})
export class PoReceipCabIdLabComponent implements OnInit {
      ...
      */
      ...
      },
      {
        ...
        onCellClick: (e: Event, args: OnEventArgs) => {
          ...
          //   this.angularGrid.gridService.deleteItem(args.dataContext);
          // }
          if (args.dataContext.prh_serial == null || args.dataContext.prh_serial == '') {
            this.hasFormErrors = true;
          ...
          if (args.dataContext.prh_part == null || args.dataContext.prh_part == '') {
            this.hasFormErrors = true;
          ...
          if (args.dataContext.prh_rcvd == 0 ) {
            this.hasFormErrors = true;
          ...
          if (args.dataContext.prh_rcvd != 0 && args.dataContext.tr_ref == null ) {
            ...
            this.addressService.getBy({ ad_addr: controls.prh_vend.value }).subscribe((response: any) => {
              //   const { data } = response;
              ...
              if (response.data != null) {
                ...
                let lab = null;
                (_lb.lb__dec01 = args.dataContext.prh_line), (_lb.lb_site = controls.prh_site.value);
                ...
                _lb.lb_um = args.dataContext.prh_um; 
                  ...
                  (reponse: any) => {
                    ...
                    let barcode = lab.lb_ref;
                     ...
                     this.index = this.dataset.findIndex((el) => {
                      return el.prh_line == args.dataContext.id;
                    });
                    this.gridService.updateItemById(args.dataContext.id, { ...args.dataContext, tr_ref: barcode, printed:true });              
  ...
  createForm() {
    ...
    this.sequenceService.getByOne({ seq_type: "PR", seq_profile: this.user.usrd_profile }).subscribe((response: any) => {
      ...
      if (this.seq) {
        ...
        this.prhreceiver = controls.prh_receiver.value
      } else {
  ...
  onSubmit(lb:any,lab:any) {
    ...
    this.data = [];
    let obj = {
      prh_line: this.dataset[this.index].prh_line,
      prh_part: this.dataset[this.index].prh_part,
      prh_desc: this.dataset[this.index].desc,
      prh_rcvd: this.dataset[this.index].prh_rcvd,
      prh_um: this.dataset[this.index].prh_um,
      prh_um_conv: this.dataset[this.index].prh_um_conv,
      prh_pur_cost: this.dataset[this.index].prh_pur_cost,
      prh_site: controls.prh_site.value,
      prh_loc: this.dataset[this.index].prh_loc,
      prh_serial: this.dataset[this.index].prh_serial,
    ...
    let pr = this.prepare();
    this.addIt(this.data, pr, this.prhreceiver,lb,lab);
   }
  ...
  //   );
  // }
  addIt(detail: any, pr, prhnbr,lb,lab) {
    ...
    this.purchaseReceiveService.addPrhCab({ detail, pr, prhnbr }).subscribe(
      (reponse: any) => {
        console.log(reponse);
  ...
  onPrint() {
    ...
    if (controls.print.value == true) this.printpdf(this.prhreceiver); 
    this.router.navigateByUrl("/purchasing/po-list");
  }
  onChangesite() {
    const controls = this.prhForm.controls;

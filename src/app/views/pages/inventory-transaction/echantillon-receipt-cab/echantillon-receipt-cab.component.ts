...
@Component({
  selector: 'kt-echantillon-receipt-cab',
  templateUrl: './echantillon-receipt-cab.component.html',
  styleUrls: ['./echantillon-receipt-cab.component.scss']
}) 


export class EchantillonReceiptCabComponent implements OnInit {
  seuil : any;
  poids:any;
  nom:any;
...
if(loc == null){loc = item.loc}
  ...
  prepareGrid4() {
    ...
    if (this.domconfig == false) {
      this.itemsService.getBy({pt_pm_code:'P'}).subscribe((response: any) => (this.items = response.data));
    } else {
      if (this.pdl == null) {
...
let date = new Date()
    ...
    var img = new Image();
    // img.src = "./assets/media/logos/echantillon-receipt-cab.png";

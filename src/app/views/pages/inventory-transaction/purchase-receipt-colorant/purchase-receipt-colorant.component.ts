...
const myCustomCheckboxFormatter: Formatter = (row: number, cell: number, value: any, columnDef: Column, dataContext: any, grid?: any) =>{
  ...
  if (value=="AUCUN"){
    return `<div class="text"  aria-hidden="AUCUN">CARTON</div>`
  }
...
@Component({
  selector: 'kt-purchase-receipt-colorant',
  templateUrl: './purchase-receipt-colorant.component.html',
  styleUrls: ['./purchase-receipt-colorant.component.scss']
}) 


export class PurchaseReceiptColorantComponent implements OnInit {
  seuil : any;
  poids:any;
  nom:any;
  ...
  initGrid() {
    this.columnDefinitions = [
      ...
      //         this.inventoryStatusService.getAllDetails({ isd_status: args.dataContext.tr_status, isd_tr_type: "RCT-PO" }).subscribe((res: any) => {
      //           console.log(res);
      //           const { data } = res;
...
if(loc == null){loc = item.loc}
  ...
  prepareGrid4() {
    ...
    if (this.domconfig == false) {
      this.itemsService.getBy({pt_pm_code:'P',pt_vend:controls.tr_addr.value,pt_draw:'COLORANT'}).subscribe((response: any) => (this.items = response.data));
    } else {
      if (this.pdl == null) {
    ...
    args.rows.map((idx) => 
    {
      ...
      this.inventoryTransactionService.getByRef({ tr_lot: item.tr_lot,tr_type:'RCT-PO',tr_effdate:item.tr_effdate,tr_addr:item.tr_addr }).subscribe(
        (res: any) => {
          ...
          controls.tr_user1.setValue(res.data[0].tr_user1)       
        },
        (error) => {

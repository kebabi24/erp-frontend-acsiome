...
@Component({
  selector: 'kt-po-unreceip-cab',
  templateUrl: './po-unreceip-cab.component.html',
  styleUrls: ['./po-unreceip-cab.component.scss']
})
export class PoUnreceipCabComponent implements OnInit {
  ...
  onSubmit() {
    ...
    let date = new Date(controls.prh_rcp_date.value ? `${controls.prh_rcp_date.value.year}/${controls.prh_rcp_date.value.month}/${controls.prh_rcp_date.value.day}` : null)
        this.addIt(this.dataset,date, this.prhnbr);
  ...
  addIt(detail: any,date,  prhnbr) {
    ...
    const controls = this.prhForm.controls;
    let poNbr = 0;
    this.purchaseReceiveService.Unreceip({ detail,date}, prhnbr ).subscribe(

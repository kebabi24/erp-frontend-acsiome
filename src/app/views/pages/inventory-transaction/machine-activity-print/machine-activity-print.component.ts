...
@Component({
  selector: 'kt-machine-activity-print',
  templateUrl: './machine-activity-print.component.html',
  styleUrls: ['./machine-activity-print.component.scss']
})
export class MachineActivityPrintComponent implements OnInit {
      ...
      // if(this.domconfig) {
        this.codeService.getByOne({code_fldname: this.user.usrd_code}).subscribe(
          (reponse: any) => { 
            ...
            this.inventoryTransactionService.getByActivity({ tr_addr: reponse.data.code_value,tr_effdate:controls.tr_effdate.value }).subscribe(
              (res: any) => {
  ...
  handleSelectedRowsChanged2(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObj2) {
      args.rows.map((idx) => {
       ...
       this.inventoryTransactionService.getByActivity({ tr_addr: item.ad_addr,tr_effdate:controls.tr_effdate.value }).subscribe(
        (res: any) => {
          this.dataset = res.data
          this.dataView.setItems(this.dataset)
...
printpdf(nbr) {
 ...
 // doc.text('This is client-side Javascript, pumping out a PDF.', 20, 30);
  var img = new Image()
  // img.src = "./assets/media/logos/machine-activity-print.png";

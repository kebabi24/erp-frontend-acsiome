...
seq: any;
        ...
        },
        {
          ...
          onCellChange: (e: Event, args: OnEventArgs) => {
            ...
            this.itemsService.getByOne({pt_part: args.dataContext.tr_part }).subscribe((resp:any)=>{
              ...
              this.locationService.getByOne({ loc_loc: resp.data.pt_loc, loc_site: resp.data.pt_site }).subscribe(
                (response: any) => {
                  ...
                  this.sctService.getByOne({ sct_site: resp.data.pt_site, sct_part: resp.data.pt_part, sct_sim: 'STD-CG' }).subscribe(
                    (response: any) => {
                      ...
                      this.gridService.updateItemById(args.dataContext.id,{...args.dataContext , desc: resp.data.pt_desc1 , tr_site:resp.data.pt_site, tr_loc:resp.data.pt_loc,
                        tr_um:resp.data.pt_um, tr_um_conv: 1,  tr_status: this.stat, tr_price: this.sct.sct_mtl_tl})
                          });     
    ...
    onChangeVend() {
      const controls = this.trForm.controls
        this.addressService
          .getBy({ ad_addr:  controls.tr_addr.value  })
...
/***********/
  ...
  doc.addPage();
      ...
      if (this.dataset[j].desc.length > 35) {
        ...
        doc.text(String((this.dataset[j].tr_price *
        this.dataset[j].tr_qty_loc).toFixed(2)), 203 , i  - 1,{ align: 'right' });
  ...
  //  doc.text(String(Number(controls.ttc.value).toFixed(2)), 198 ,  i + 33 , { align: 'right' });
   ...
   doc.setFontSize(8);
      let mt = NumberToLetters(
        Number(total).toFixed(2),"Dinars Algerien")

...
@Component({
  selector: "kt-business-monitoring-sheet",
  templateUrl: "./business-monitoring-sheet.component.html",
  styleUrls: ["./business-monitoring-sheet.component.scss"],
})
export class BusinessMonitoringSheetComponent implements OnInit {
  addReport: AddReport;
  empForm: FormGroup;
  row_number;
  ...
  addDet(pme: any) {
    ...
    this.projectService.sendDataToModelSuivi({ pme: pme }).subscribe(
  ...
  handleSelectedRowsChanged(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObjpm) {
      args.rows.map((idx) => {
        ...
        this.projectService.getByTask({ pmt_code: this.pmcode }).subscribe((response: any) => ((this.dataset = response.data["details"]), console.log(this.dataset)));

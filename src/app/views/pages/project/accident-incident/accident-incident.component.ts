...
export class AccidentIncidentComponent implements OnInit {
  ...
  site_name : any;
    ...
    onSubmit() {
        ...
        this.defaultData.forEach(element => {
          ...
          lines.push({
            mph_part: this.site_code,
            //mph_routing  :code specification 
            mph_mch : this.emp_code,
            mph_pass : element.checked,  
        ...
        this.qualityControlService.createTestHistory(lines).subscribe(
          ...
          () => {
              ...
              const fileterd = _.mapValues(_.groupBy(this.defaultData, 'mpd_chr01'));  
              const filtered_elements = [];
              for (const [key, value] of Object.entries(fileterd)) {
                filtered_elements.push({
                  group_title: key,
  ...
  handleSelectedRowsChanged4(e, args) {
    ...
    this.emp_code = this.gridService4.getDataItemByRowIndex(this.selectedIndexesEmp[0]).emp_addr
    // this.prepareGrid4(this.site_code)
    console.log(this.emp_code)
    const controls = this.accidentForm.controls
    controls.emp_code.setValue(this.emp_code || "")
    const empName = this.gridService4.getDataItemByRowIndex(this.selectedIndexesEmp[0]).emp_fname +' '+this.gridService4.getDataItemByRowIndex(this.selectedIndexesEmp[0]).emp_lname
  ...
  onCheckClick(label,group_title){
   ...
   const elementIndex = this.data[groupIndex].elements.findIndex(element =>{
    return element.mpd_label == label
   })

...
export class CreateNewRolesComponent implements OnInit {
  ...
  createForm() {
    ...
    this.roleForm = this.roleF.group({
        ...
        role_userMobileId: [{value: this.role.role_userMobileId, disabled: !this.isExist}, Validators.required],
    })
  }
  ...
  onChangeCode() {
    ...
    this.roleService.getByOne({role_name: controls.role_name.value }).subscribe(
        (res: any) => {
          console.log("aa", res.data);
       
          if (res.data) {
            alert("Ce role exist déja")
            controls.role_name.setValue(null) 
            document.getElementById("role").focus(); 

          } else { 

            controls.role_userMobileId.enable()
...
// save data
  ...
  prepareRole(): Role {
    ...
    _role.role_userMobileId = controls.role_userMobileId.value
  ...
  handleSelectedRowsChanged(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObj) {
        args.rows.map((idx) => {
            ...
            controls.role_userMobileId.setValue(item.id || "")
        })
    }
  }

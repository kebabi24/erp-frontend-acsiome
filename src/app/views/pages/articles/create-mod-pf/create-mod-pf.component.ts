...
@Component({
  selector: 'kt-create-mod-pf',
  templateUrl: './create-mod-pf.component.html',
  styleUrls: ['./create-mod-pf.component.scss']
})



export class CreateModPfComponent implements OnInit {
  ...
  goBack() {
    this.loadingSubject.next(false);
    const url = `/articles/create-mod-pf`;
         ...
         console.log(response.data)
          if (response.data != null) {
            ...
            this.codeService.getBy({ code_fldname: 'pt_draw',code_value: controls.mod_draw.value }).subscribe((drawresponse: any) => {code_draw = drawresponse.data[0].chr01
              ...
              if(controls.mod_group.value == 'BROYE')
              {
                controls.mod_dsgn_grp.setValue('BROY')
                controls.mod_origin.setValue('BROYAGE')
              }
              else
              {
                controls.mod_dsgn_grp.setValue('N/BROY')
                if(controls.mod_draw.value == 'SQUELETTE'){controls.mod_origin.setValue('MCH')}
                else{controls.mod_origin.setValue('ACH')}
              }
  ...
  addItem(item: ItemModel) {
    ...
    this.itemModelService.add(item).subscribe(
      ...
      () => {
                ...
                this.loadingSubject.next(false);
                this.reset()
                this.router.navigateByUrl("/articles/create-mod-pf");
              }
            );

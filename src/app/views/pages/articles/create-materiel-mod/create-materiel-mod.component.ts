...
export class CreateMaterielModComponent implements OnInit {
  ...
  constructor(
    ...
    this.prepareGrid2();
    this.codeService
      .getBy({ code_fldname: "pt_model" })
      .subscribe((response: any) => (this.pt_model = response.data));
    this.codeService
  ...
  createForm() {
    ...
    this.form1 = this.formBuilder.group({
      ...
      pt_model: [{value: this.item.pt_model,disabled: true },Validators.required ],
      pt_part: [{value:this.item.pt_part},Validators.required],
      pt_desc1: [{ value: this.item.pt_desc1 },Validators.required],
      ...
      pt_length: [{ value: this.item.pt_length }],
      pt_width: [{ value: this.item.pt_width }],
      pt_height: [{ value: this.item.pt_height }],
      pt_dim_um: [{ value: this.item.pt_dim_um }],
      pt_ll_code: [{ value: this.item.pt_ll_code }],
      pt_ll_drp: [{ value: this.item.pt_ll_drp }],
      
      chr01: [{ value: '' }],
      chr02: [{ value: this.item.chr02 }],
      chr03: [{ value: this.item.chr03 }],
      dec01: [{ value: this.item.dec01 }],
      pt_userid: [{ value: this.item.pt_userid }],
      pt_status: [{ value: this.item.pt_status }],
      
      int02: [{ value: this.item.int02 }],
      pt_net_wt: [{ value: this.item.pt_net_wt }],
      int03: [{ value: this.item.int03 }],
  ...
  onChangeCode() {
    ...
    let index = this.pt_model.findIndex(x => x.code_value == controls1.pt_model.value); 
    console.log(controls1.pt_article.value)
    this.codedesc = this.pt_model[index].code_desc
    const codecmmt = this.pt_model[index].code_cmmt
    this.itemService
        .getByOne({
            pt_article: controls1.pt_article.value,
            pt_model: controls1.pt_model.value,
        ...
        .subscribe((modeleresponse: any) => {
         console.log(modeleresponse.data)
          if (modeleresponse.data) {
            ...
            let vitesse:any;
            this.codeService.getBy({ code_fldname: 'pt_model',code_value: controls1.pt_model.value }).subscribe((coderesponse: any) => 
              { code_couleur = coderesponse.data[0].code_desc, vitesse=Number(coderesponse.data[0].dec01)
                ...
                controls1.chr01.setValue(null)
                controls1.chr02.setValue(null)
                controls1.chr03.setValue(null)
                controls1.pt_userid.setValue(null)
                controls1.pt_part.setValue(controls1.pt_article.value + code_couleur )
                controls1.pt_desc1.setValue(controls1.pt_draw.value + " " + controls1.pt_part_type.value + " " + controls1.pt_group.value + " " + controls1.pt_model.value)
...
onAlertClose($event) {
  ...
  onSubmit() {
    ...
    if(controls1.pt_draw.value == 'MOULE')
      {let usage = this.prepareUsage()
        this.addCode(usage)
      }
  ...
  prepareItem(): Item {
    ...
    _item.pt_dim_um = controls1.pt_dim_um.value;
    _item.pt_ll_code = controls1.pt_ll_code.value;
    _item.pt_ll_drp = controls1.pt_ll_drp.value;

    _item.chr01 = controls1.chr01.value;
    _item.chr02 = controls1.chr02.value;
    _item.chr03 = controls1.chr03.value;
    _item.dec01 = controls1.dec01.value;
    _item.pt_userid = controls1.pt_userid.value;
  ...
  prepareUsage(): Code {
    ...
    _code.code_value = controls.pt_part.value
   _code.code_desc = 'MATERIEL'
    _code.code_cmmt = controls.pt_desc1.value
    ...
    this.itemModelService.getBy({ mod_code}).subscribe(
      (res: any) => {
        ...
        if (!data) {
          ...
          controls.pt_model.setValue(null)
          controls.pt_model.enable()

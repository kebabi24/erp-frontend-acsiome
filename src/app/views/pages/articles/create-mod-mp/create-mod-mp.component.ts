...
@Component({
  ...
  templateUrl: './create-mod-mp.component.html',
  styleUrls: ['./create-mod-mp.component.scss']
})

export class CreateModMpComponent implements OnInit {
  ...
  goBack() {
    this.loadingSubject.next(false);
    const url = `/articles/create-mod-mp`;
  ...
  addItem(item: ItemModel) {
    ...
    this.itemModelService.add(item).subscribe(
      ...
      () => {
                ...
                this.loadingSubject.next(false);
                this.reset()
                this.router.navigateByUrl("/articles/create-mod-mp");
              }
            );

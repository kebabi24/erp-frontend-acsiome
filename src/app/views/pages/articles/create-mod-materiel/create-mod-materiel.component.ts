...
@Component({
  selector: 'kt-create-mod-materiel',
  templateUrl: './create-mod-materiel.component.html',
  styleUrls: ['./create-mod-materiel.component.scss']
})

export class CreateModMaterielComponent implements OnInit {
  ...
  constructor(
    ...
    this.codeService
      .getBy({ code_fldname: "pt_prod_line",code_desc:'MATERIEL' })
      .subscribe((response: any) => (this.mod_prod_line = response.data));
    this.codeService
      .getBy({ code_fldname: "pt_part_type",code_desc:'MATERIEL' })
      .subscribe((response: any) => (this.mod_part_type = response.data));
    this.codeService
      .getBy({ code_fldname: "pt_draw",code_desc:'MATERIEL' })
      .subscribe((response: any) => (this.mod_draw = response.data));
    this.codeService
      .getBy({ code_fldname: "pt_group", code_desc: 'MATERIEL' })
      .subscribe((response: any) => (this.mod_group = response.data));  
    this.codeService
      .getBy({ code_fldname: "pt_promo",code_desc:'MATERIEL' })
      .subscribe((response: any) => (this.mod_promo = response.data));
      this.codeService
      .getBy({ code_fldname: "pt_dsgn_grp",code_desc:'MATERIEL' })
      .subscribe((response: any) => (this.mod_dsgn_grp = response.data));
    this.codeService
      .getBy({ code_fldname: "pt_origin",code_desc:'MATERIEL' })
      .subscribe((response: any) => (this.mod_origin = response.data));  
    this.codeService
      .getBy({ code_fldname: "pt_drwg_loc",code_desc:'MATERIEL' })
  ...
  goBack() {
    this.loadingSubject.next(false);
    const url = `/articles/create-mod-materiel`;
  ...
  createForm() {
    ...
    this.form1 = this.formBuilder.group({
      ...
      mod_code: [{value :this.itemModel.mod_code}],
      mod_desc: [{value :this.itemModel.mod_desc }, ],
      mod_um: [{value :this.itemModel.mod_um },],
      mod_promo: [{value :this.itemModel.mod_promo}],
      mod_dsgn_grp: [{value :this.itemModel.mod_dsgn_grp}],
      mod_origin: [{value :this.itemModel.mod_origin}],
  ...
  onChangepromo() {
    ...
    this.itemModelService
        .getByOne({
              ...
              mod_origin:controls.mod_origin.value,
              mod_promo:controls.mod_promo.value,
        })
        .subscribe((response: any) => {
         console.log(response.data)
          if (response.data != null) {
            ...
            this.codeService.getBy({ code_fldname: 'pt_draw',code_value: controls.mod_draw.value }).subscribe((drawresponse: any) => {code_draw = drawresponse.data[0].chr01
              this.codeService.getBy({ code_fldname: 'pt_part_type',code_value: controls.mod_part_type.value }).subscribe((typeresponse: any) => {code_type = typeresponse.data[0].chr01
                this.codeService.getBy({ code_fldname: 'pt_group',code_value: controls.mod_group.value }).subscribe((grouperesponse: any) => {code_group = grouperesponse.data[0].chr01
                  this.codeService.getBy({ code_fldname: 'pt_promo',code_value: controls.mod_promo.value }).subscribe((promoresponse: any) => {code_promo = promoresponse.data[0].chr01
                    this.codeService.getBy({ code_fldname: 'pt_origin',code_value: controls.mod_origin.value }).subscribe((originresponse: any) => {code_origin = originresponse.data[0].chr01
                 
                    controls.mod_code.setValue(controls.mod_prod_line.value + code_type + code_draw +  code_group + code_promo + code_origin )
                    controls.mod_desc.setValue(controls.mod_draw.value + ' ' + controls.mod_part_type.value + ' ' + controls.mod_group.value + ' ' +  controls.mod_promo.value + ' ' +controls.mod_origin.value )
                    controls.mod_um.setValue('UN')
                    });
                  });
                });
              });
  ...
  prepareItem(): ItemModel {
    ...
    _itemModel.mod_promo = controls1.mod_promo.value;
    ...
    _itemModel.mod_status = 'DV-ACTIF';
...
prepareUsage(): Code {
  const controls = this.form1.controls
  const _code = new Code()
  _code.code_fldname = 'pt_origin'
  _code.code_value = controls.mod_code.value
 _code.code_desc = 'MATERIEL'
  _code.code_cmmt = controls.mod_draw.value
  ...
  onSubmit() {
    ...
    // }
    if(controls1.mod_group.value == null){
      this.message = 'Veuillez choisir un groupe article'
      this.hasFormErrors1 = true
      return;
    }
  ...
  addItem(item: ItemModel) {
    ...
    this.itemModelService.add(item).subscribe(
      ...
      () => {
                ...
                this.loadingSubject.next(false);
                this.reset()
                this.router.navigateByUrl("/articles/create-mod-materiel");
              }
            );

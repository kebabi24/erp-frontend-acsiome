...
const myCustomCheckmarkFormatter: Formatter = (
 ...
 @Component({
  selector: 'kt-alter-part',
  templateUrl: './alter-part.component.html',
  styleUrls: ['./alter-part.component.scss']
})
export class AlterPartComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.createForm();
    this.codeService
    .getBy({ code_fldname: "articles/alter-part" })
...
onAlertClose($event) {
  ...
  addItem(item: Item) {
    ...
    this.itemService.alter(item).subscribe(
...
handleSelectedRowsChangedmod(e, args) {
  ...
  if (Array.isArray(args.rows) && this.gridObjmod) {
    args.rows.map((idx) => {
      ...
      controls1.pt_part.setValue(item.pt_part || "");
      this.model = item
      controls1.pt_prod_line.enable()
          
          
          controls1.pt_prod_line.setValue(item.pt_prod_line)
          controls1.pt_fr_class.setValue(item.pt_fr_class)
          controls1.pt_part_type.setValue(item.pt_part_type)
          controls1.pt_draw.setValue(item.pt_draw)
          controls1.pt_group.setValue(item.pt_group)
          controls1.pt_rev.setValue(item.pt_rev)
          controls1.pt_break_cat.setValue(item.pt_break_cat)
          controls1.pt_desc1.setValue(item.pt_desc1)
            controls1.pt_desc2.setValue(item.pt_desc2)
            controls1.pt_um.setValue(item.pt_um)
            controls1.pt_price.setValue(item.pt_price)
            controls1.pt_plan_ord.setValue(item.pt_plan_ord)
            controls1.pt_added.setValue(item.pt_added)
            controls1.pt_promo.setValue(item.pt_promo)
            
            controls1.pt_dea.setValue(item.pt_dea)
            controls1.pt_origin.setValue(item.pt_origin)
            controls1.pt_bom_code.setValue(item.pt_bom_code)
            controls1.pt_dsgn_grp.setValue(item.pt_dsgn_grp)
            controls1.pt_status.setValue(item.pt_status)
           
            
            
            controls1.pt_part.disable()
            
  controls1.pt_length.setValue(item.pt_length)
  controls1.pt_width.setValue(item.pt_width)
  controls1.pt_height.setValue(item.pt_height)
  controls1.pt_size.setValue(item.pt_size)
  controls1.pt_net_wt.setValue(item.pt_net_wt)
...
doc.setFontSize(14);
          ...
          doc.text("Modèle: " + controls.pt_rev.value, 7, 55);
          ...
          doc.text("Formule: " + controls.pt_bom_code.value, 7, 95);
          doc.text("Origine: " + controls.pt_origin.value, 7, 100);
          ...
          doc.text("Forme: " + controls.pt_dsgn_grp.value, 7, 110);
          doc.text("Statut: " + controls.pt_status.value, 7, 115);
          doc.text("DA Obligatoire: " + controls.pt_plan_ord.value, 7, 120);
          doc.text("Achat: " + controls.pt_dea.value, 55, 120);

...
@Component({
  selector: 'kt-create-pf-mod', 
  templateUrl: './create-pf-mod.component.html',
  styleUrls: ['./create-pf-mod.component.scss']
})
export class CreatePfModComponent implements OnInit {
  ...
  constructor(
    ...
    this.codeService
      .getBy({ code_fldname: "pt_net_wt" })
      .subscribe((response: any) => (this.pt_net_wt = response.data));  
    this.codeService
  ...
  createForm() {
    ...
    this.form1 = this.formBuilder.group({
      ...
      pt_net_wt:[{value: this.item.pt_net_wt,disabled: true },Validators.required ],
      ...
      pt_plan_ord: [{ value: false , disabled: !this.isExist}],
      pt_dea: [{ value: false , disabled: !this.isExist}],
      pt_model: [""],
      pt_vend: [""],
...
onAlertClose($event) {
  ...
  prepareItem(): Item {
    ...
    _item.pt_net_wt = Number(controls1.pt_net_wt.value);
    _item.pt_abc = 'C';
    _item.pt_site = '1000';
...
printpdf() {
          ...
          doc.setFontSize(10);
          doc.text("Description: " + controls.pt_desc1.value, 7, 45);
          doc.text("Modèle: " + controls.pt_article.value, 7, 50);
          doc.text("Couleur: " + controls.pt_break_cat.value, 7, 55);
          ...
          doc.text("Categorie: " + controls.pt_prod_line.value, 7, 65);
          doc.text("Type: " + controls.pt_part_type.value, 7, 70);
          doc.text("Famille: " + controls.pt_draw.value, 7, 75);
          doc.text("Sous-Famille: " + controls.pt_group.value, 55, 80);
          doc.line(5, 85, 200, 85);
          
          // doc.text("Prix: " + controls.pt_price.value, 55, initialY + 30);
          
          doc.text("Formule: " + controls.pt_bom_code.value, 7, 90);
          doc.text("Origine: " + controls.pt_origin.value, 7, 95);
          // doc.text("Fournisseur: " + controls.pt_vend.value, 7, 100);
          // doc.text("Réference: " + controls.pt_model.value, 55, 105);
          doc.line(5, 110, 200, 110);
          
          // doc.text("Fournisseur: " + controls.pt_vend.value, 5, initialY + 80);
          
          doc.text("Forme: " + controls.pt_dsgn_grp.value, 7, 115);
          doc.text("Statut: " + controls.pt_status.value, 7, 120);
          doc.text("DA Obligatoire: " + controls.pt_plan_ord.value, 7, 125);
          doc.text("Achat: " + controls.pt_dea.value, 55, 125);

...
@Component({
  selector: 'kt-edit-solde-equipement',
  templateUrl: './edit-solde-equipement.component.html',
  styleUrls: ['./edit-solde-equipement.component.scss']
})
export class EditSoldeEquipementComponent implements OnInit {
  ...
  /*    {
      ...
      },
      {
        ...
        type: FieldType.text,
        filter: {
          collection: [  { value: 'E', label: 'E' }, { value: 'C', label: 'C' } ],
      ...
      }, 
      {
        ...
        grouping: {
          getter: 'equip',
          formatter: (g) => `Equipement: ${g.value}  <span style="color:green">(${g.count} items)</span>`,
          aggregators: [
          new Aggregators.Sum('debit'),  
          new Aggregators.Sum('credit'),  
  ...
  solist() {
    ...
    this.bankService.getBkhCautionDetail(obj).subscribe(
      (response: any) => {   
        this.mvdataset = response.data.result
       console.log(this.mvdataset)
       controls.solde.setValue(response.data.solde)
...
console.log("date",date1)
    ...
    doc.text("Equipement", 90, 58.5);
    doc.line(115, 55, 115, 60);
    doc.text("Date", 117, 58.5);
    doc.line(140, 55, 140, 60);
    
    doc.text("Observation", 152, 58.5);
    doc.line(220, 55, 220, 60);
    doc.text("Type", 222, 58.5);
    doc.line(232, 55, 232, 60);
    doc.text("Débit", 240, 58.5);
    doc.line(260, 55, 260, 60);
    doc.text("Crédit", 270, 58.5);
    ...
    // if(this.mvdataset[j].credit != null) {
      let montantcredit = 0 
      if(Number(this.mvdataset[j].credit) != 0) { montantcredit = Number(this.mvdataset[j].credit)} else { montantcredit = 0}
    ...
    // }
      ...
      if (j % 27 == 0 && j != 0) {
       ...
       if (bool) { doc.text(controls.cust.value, 7.5, i - 1);
        ...
        doc.text(String(this.mvdataset[j].equip) , 77, i - 1);
        doc.line(115, i - 5, 115, i);
       if(this.mvdataset[j].date != null) doc.text(String(this.mvdataset[j].date) , 117, i - 1);
       doc.line(140, i - 5, 140, i);
        if (this.mvdataset[j].note != null) doc.text(String(this.mvdataset[j].note) , 142, i - 1);
        doc.line(220, i - 5, 220, i);
        if (this.mvdataset[j].type != null) doc.text(String(this.mvdataset[j].type) , 225, i - 1);
        doc.line(232, i - 5, 232, i);


       if(this.mvdataset[j].debit != null) doc.text(mndeb , 258, i - 1,{ align: "right" });
       doc.line(260, i - 5, 260, i);
       if(this.mvdataset[j].credit != null) doc.text(mncre , 288, i - 1,{ align: "right" });
     ...
     let tnb = replaceAll(nb,","," ")
        doc.text(tnb, 258, i - 1,{ align: "right" });
        console.log("tnb",tnb)
        doc.line(260, i - 5, 260, i);
       ...
       console.log(nbm)
        ...
        doc.text(tnbcr, 288, i - 1,{ align: "right" });
        ...
        let sol =   Number(totaldebit) - Number(totalcredit)
      
         let sold =  String(sol.toLocaleString("en-US", {
           minimumFractionDigits: 2,
           maximumFractionDigits: 2,
         }))
         console.log("nb",nb)
      let solde = replaceAll(sold,","," ")
         doc.text("Solde : " + solde, 290, i - 1,{ align: "right" });

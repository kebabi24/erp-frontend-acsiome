...
export class AffectEmpComponent implements OnInit {
  ...
  constructor(config: NgbDropdownConfig, private empFB: FormBuilder, private activatedRoute: ActivatedRoute, private router: Router, public dialog: MatDialog, private layoutUtilsService: LayoutUtilsService, private modalService: NgbModal, private employeService: EmployeService, private affectEmpService: AffectEmpService, private codeService: CodeService, private siteService: SiteService, private taskService: TaskService, private projectService: ProjectService, private providerService: ProviderService) {
  ...
  createForm() {
    ...
    this.empForm = this.empFB.group({
      ...
      pme_inst: [this.affectEmp.pme_inst, Validators.required],
      pme_task: [this.affectEmp.pme_task],
      taskdesc: [{ value: "", disabled: true }],

      pme_task_status: [{ value: this.affectEmp.pme_task_status, disabled: true }],
...
prepareCode(): any {
  ...
  prepareGridinst() {
    ...
    // fill the dataset with your data
    this.projectService.getBy({ pm_code: controls.pme_pm_code.value }).subscribe((response: any) => (this.datasetinst = response.data.details));
  }
  openinst(content) {
...
{         
    ...
    // fill the dataset with your data
    this.projectService.getByTask({ pmt_code: controls.pme_pm_code.value, pmt_inst: controls.pme_inst.value }).subscribe((response: any) => (this.datasettask = response.data.details));
  }
  opentask(content) {
    this.prepareGridtask();
    this.modalService.open(content, { size: "lg" });
  ...
  //       this.employeService.getByJob({ empj_addr: item.emp_addr, empj_job: this.job, empj_level: this.level }).subscribe((response: any) => {
  //         //  if (response.data.length = 0)

...
@Component({
  selector: 'kt-bobine-issue',
  templateUrl: './bobine-issue.component.html',
  styleUrls: ['./bobine-issue.component.scss']
})

export class BobineIssueComponent implements OnInit {
  ...
  onChangeVend() {
   ...
   //   const { data } = response;
      ...
      if (response.data == null) {
        ...
        if(controls.tr_addr.value != 'M1' && controls.tr_addr.value != 'M2' && controls.tr_addr.value != 'M3'){
          this.layoutUtilsService.showActionNotification("vous ne pouvez pas consommer cette bobin sur cette!", MessageType.Create, 10000, true, true);
          this.error = true;
        }
        else{
          this.provider = response.data;
...
this.labelService.getBy({lb_cab: ref,lb_actif: false}).subscribe((res:any) =>{if (res.data != null) {bol = true}})
  ...
  if (controls.tr_addr.value == null){  controls.ref.setValue(null)
    this.message = "veuillez saisir l'adresse";
  this.hasFormErrors = true;
  return;}
  else{
    ...
    if (this.lddet != null) {
     ...
     this.itemsService.getByOne({pt_part: this.lddet.ld_part,pt_draw:'BOBINE'  }).subscribe(
      (respopart: any) => {
        console.log(respopart)
        if(respopart.data == null){
              this.message = "bobine inconnue";
              this.hasFormErrors = true;
              return;
        }
        else{
  ...
  //  this.gridService.updateItemById(args.dataContext.id,{...args.dataContext , tr_part: null })
    ...
    this.message = "bobine déjà scanné";
    this.hasFormErrors = true;
    return;
  }
}
...
doc.addPage();
    ...
    if (this.dataset[j].desc.length > 35) {
      ...
      doc.text(String(Number(this.dataset[j].tr_price) * Number(this.dataset[j].tr_qty_loc)), 203, i - 1, { align: "right" });

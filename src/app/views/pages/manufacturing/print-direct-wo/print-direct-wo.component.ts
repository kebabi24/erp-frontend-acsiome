...
@Component({
  selector: 'kt-print-direct-wo',
  templateUrl: './print-direct-wo.component.html',
  styleUrls: ['./print-direct-wo.component.scss']
})

export class PrintDirectWoComponent implements OnInit {
  seuil:any;
  ...
  createForm() {
    ...
    this.woForm = this.woFB.group({
      ...
      wo_lot: [this.workOrder.wo_lot, Validators.required],
      wo_nbr: [this.workOrder.wo_nbr, Validators.required],
  ...
  prepareGrid4() {
    ...
    this.itemsService.getProd({pt_group:'BROYE'}).subscribe((response: any) => (this.items = response.data));
  }
  open4(content) {
    this.prepareGrid4();
    this.modalService.open(content, { size: "lg" });
  ...
  printpdf(nbr) {
   ...
   // doc.text('This is client-side Javascript, pumping out a PDF.', 20, 30);
    ...
    doc.text("Date : " + this.trdate, 160, 45);
    ...
    doc.text("Employés          : " + controls.wo_user1.value + ' ' + controls.wo_user2.value, 120, 55);
    doc.text("Couleur Produit   : " + controls.product_color.value, 20, 60);
    doc.text("Quantité sortie   : " + controls.wo_qty_comp.value, 20, 65);
    doc.text("Lot N°            : " + controls.wo_serial.value, 20, 70);
...
prepareGrid5() {
  ...
  // fill the dataset with your data
  const controls = this.woForm.controls;
  console.log(controls.wo_routing.value)
  this.workOrderService.getBy({ wo_status: "C", wo_routing: controls.wo_routing.value }).subscribe((response: any) => {
    // console.log(response.data)
...
handleSelectedRowsChanged5(e, args) {
  ...
  if (Array.isArray(args.rows) && this.gridObj5) {
    args.rows.map((idx) => {
      ...
      controls.product_type.setValue(item.item.pt_draw);
      controls.product_color.setValue(item.item.pt_break_cat);
      controls.wo_user1.setValue(item.wo_user1);
      controls.wo_user2.setValue(item.wo_user2)
      controls.wo_qty_comp.setValue(item.wo_qty_comp)
      this.inventoryTransactionService.getByRef({tr_domain: this.domain,tr_type:'ISS-WO',tr_nbr:item.wo_nbr}).subscribe(
        (res: any) => {        
      ...
      this.inventoryTransactionService.getByRef({tr_domain: this.domain,tr_type:'RCT-WO',tr_nbr:item.wo_nbr}).subscribe(
        (res: any) => {    
          for (let prod of res.data){this.trdate = prod.tr_effdate;controls.wo_serial.setValue(prod.tr_serial)}   

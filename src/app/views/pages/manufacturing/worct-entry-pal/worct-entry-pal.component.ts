...
export class WorctEntryPalComponent implements OnInit {
...

      {
        ...
        onCellChange: (e: Event, args: OnEventArgs) => {
          ...
          this.siteService.getByOne({ si_site: args.dataContext.tr_site }).subscribe((response: any) => {
            ...
            if (response.data) {
              this.sctService.getByOne({ sct_site: response.data.si_site, sct_part: controls.tr_part.value, sct_sim: "STD-CG" }).subscribe((resp: any) => {
                this.sct = resp.data;
  ...
  onSubmit() {
    ...
    _lb.lb_part = controls.tr_part.value;
    _lb.lb_nbr = controls.tr_nbr.value;
    (_lb.lb_lot = this.uniquelot == "L" ? `${this.product.pt_article}${this.product.pt_break_cat}${this.product.pt_net_wt}/${days}.${months}.${year}` : null), (_lb.lb_date = controls.tr_effdate.value ? `${controls.tr_effdate.value.year}/${controls.tr_effdate.value.month}/${controls.tr_effdate.value.day}` : null);
    _lb.lb_qty = qtypal;
  ...
  handleSelectedRowsChangedsite(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObjsite) {
      args.rows.map((idx) => {
        ...
        this.sctService.getByOne({ sct_site: item.si_site, sct_part: controls.tr_part.value, sct_sim: "STD-CG" }).subscribe((resp: any) => {
          this.sct = resp.data;
  ...
  handleSelectedRowsChangedloc(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObjloc) {
      args.rows.map((idx) => {
        ...
        this.locationService.getByOne({ loc_loc: item.loc_loc, loc_site: controls.tr_site.value }).subscribe((response: any) => {
          this.location = response.data;
          if (response.data) {
  ...
  handleSelectedRowsChanged5(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObj5) {
      args.rows.map((idx) => {
        ...
        this.sctService.getByOne({ sct_site: this.site, sct_part: controls.tr_part.value, sct_sim: "STD-CG" }).subscribe((resp: any) => {
          this.sct = resp.data;
          console.log(this.sct);
        });
      });
  ...
  prepareGrid5() {
    ...
    // fill the dataset with your data
    this.workOrderService.getBy({ wo_status: "R" }).subscribe((response: any) => {
      // console.log(response.data)
  ...
  onChangeLoc() {
    ...
    this.locationService.getByOne({ loc_site, loc_loc }).subscribe((res: any) => {
      //console.log(res)

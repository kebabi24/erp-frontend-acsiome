...
import { ItemService, AddressService, SequenceService, InventoryTransaction, InventoryTransactionService, InventoryStatusService, SiteService, LocationService, LocationDetailService, CostSimulationService, CodeService, MesureService, WorkOrderService, Label, LabelService, SaleOrderService, PrintersService, Daybook } from "../../../../core/erp";
  ...
  handleSelectedRowsChangedloc(e, args) {
    ...
    if (Array.isArray(args.rows) && this.gridObjloc) {
      args.rows.map((idx) => {
        ...
        this.locationService.getByOne({ loc_loc: item.loc_loc, loc_site: updateItem.tr_site }).subscribe((response: any) => {
          this.location = response.data;
          if (response.data) {
